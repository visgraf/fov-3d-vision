"""Blender-side acquisition of one FSG4c fixation pair.

The scene is selected by an opaque fixture id. Render seeds are keyed by
(fixture, Monte-Carlo seed, yaw, eye), not by policy or step, so active and scan
receive byte-identical stochastic observations whenever they visit the same yaw.
"""
...
import fsg4c_scene as scene_spec
...
def acquire_one(args: argparse.Namespace) -> dict:
    if args.fixture not in public.FIXTURES:
        raise ValueError("fixture not in frozen FSG4c schedule")
    ...
    if args.step < 0 or args.step >= public.MAX_BUDGET_FIXATIONS:
        raise ValueError("step outside frozen FSG4 budget")
    public.yaw_lattice_index(args.yaw)
    ...
    configure_backend(args.fixture)
    ...
    scene_spec.validate_mesh(args.fixture, mesh)
    obs = {}
    seconds = []
    seeds = []
    t_oracle = 0.0
    ...
    for eye_id, eye in enumerate(calibration["eyes"]):
        render_seed = public.render_seed(args.fixture, args.seed, args.yaw, eye_id)
    ...
    acq = {
        "schema": "FSG4c-fixation-acquisition-v1",
        ...
        "segmentation": "oracle first-hit Blender pass_index; not antialiased",
        "checks": backend.checks,
    }
    json_write(folder / "acquisition.json", acq)
    run = {
        "schema": "FSG4c-fixation-run-v1",
        "source": backend.source,
        "complete": True,
        ...
        "truth_spec_sha256": scene_spec.truth_digest(args.fixture),
        "total_wall_seconds": time.perf_counter() - t_all,
    }
    json_write(out / "run.json", run)
    print(f"[fsg4c-render] COMPLETE fixture={args.fixture} seed={args.seed} step={args.step} yaw={args.yaw:.3f} samples={acq['primary_camera_samples']}", flush=True)
    return run
...
if __name__ == "__main__":
    try:
        ...
        if isinstance(exc, SystemExit) and exc.code in (0, None):
            raise
        traceback.print_exc()
        print("[fsg4c-render] FAILED", flush=True)
