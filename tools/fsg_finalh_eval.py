...
def draw(dest: Path, candidate: dict, ctx: dict, core: np.ndarray) -> None:
    ...
    d.text((8,8),"FSG1h FINAL PROSPECTIVE VALIDATION",fill="black")
...
def evaluate_pair(folder: Path, dest: Path, meta: dict, allow_synthetic: bool) -> dict:
    ...
    for key,m in metrics["metrics"].items():
        if key=="interior" or key.startswith("instance_") or key=="boundary":
            print(f"[fsg-finalh] {meta['profile']}/seed{meta['seed']}/{folder.name}/{key} reference={m['reference_pixels']} coverage={m['coverage']} median={m['median_relative_range_error']} p95={m['p95_relative_range_error']}")
    print(f"[fsg-finalh] {folder.name} occlusion_raw={occ['raw_reference']} core={occ['core_reference']} accepted_core={occ['accepted_core']}")
    for x in fails: print(f"[fsg-finalh] NUMERICAL_FAIL {folder.name}: {x}")
    return report
...
def compare(runs: list[Path],out: Path,mode: str,allow_synthetic: bool=False) -> dict:
    ...
    status=("SYNTHETIC_FINAL_VALIDATION_NOT_A_RENDER_RESULT" if not all_real else
            ...
            "FSG1H_FINAL_VALIDATION_PASS" if passed else "FSG1H_FINAL_VALIDATION_FAIL")
    result={"schema":"FSG1h-final-validation-v1","status":status,"mode":mode,"candidate_id":CANDIDATE_ID,
    ...
    print(f"[fsg-finalh] SUMMARY mode={mode} runs={len(runs)} checks_pass={str(passed).lower()} status={status} increment2_authorized={str(final_pass).lower()}")
    return result
...
if __name__=="__main__":
    try: main()
        ...
        print(f"[fsg-finalh] FAIL {type(exc).__name__}: {exc}",file=sys.stderr);raise SystemExit(1)
