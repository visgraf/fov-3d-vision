"""Evaluator-only geometry for Stage II / Scene-1c certified composition.

Both fresh fixtures contain three angularly disjoint compact convex ribbons in
an FSG6f-like geometry family.  This module provides only rendering/evaluation
geometry and prospective design sanity checks.  It does NOT claim the actors are
solvable: that claim is made only if the twelve full component controls pass.
Prediction-side modules must not import this file.
"""
...
FIXTURE = {
    "cert_triad_e": {
        201: {"type": "cylinder", "yaw_deg": -15.0, "pitch_deg": +8.0, "roll_deg": +10.0,
              "centre_z_local_m": -2.83, "radius_m": 0.75, "theta_min_deg": -26.0,
              "theta_max_deg": +26.0, "height_m": 0.18, "texture_tag": 71},
        202: {"type": "cylinder", "yaw_deg": 0.0, "pitch_deg": -9.0, "roll_deg": -12.0,
              "centre_z_local_m": -2.81, "radius_m": 0.73, "theta_min_deg": -25.0,
              "theta_max_deg": +25.0, "height_m": 0.18, "texture_tag": 73},
        203: {"type": "cylinder", "yaw_deg": +15.0, "pitch_deg": +8.0, "roll_deg": +15.0,
              "centre_z_local_m": -2.86, "radius_m": 0.76, "theta_min_deg": -25.0,
              "theta_max_deg": +25.0, "height_m": 0.18, "texture_tag": 79},
    },
    "cert_triad_f": {
        201: {"type": "cylinder", "yaw_deg": 0.0, "pitch_deg": -8.0, "roll_deg": +14.0,
              "centre_z_local_m": -2.82, "radius_m": 0.74, "theta_min_deg": -25.0,
              "theta_max_deg": +25.0, "height_m": 0.18, "texture_tag": 83},
        202: {"type": "cylinder", "yaw_deg": -15.0, "pitch_deg": +9.0, "roll_deg": -10.0,
              "centre_z_local_m": -2.84, "radius_m": 0.75, "theta_min_deg": -25.0,
              "theta_max_deg": +25.0, "height_m": 0.18, "texture_tag": 89},
        203: {"type": "cylinder", "yaw_deg": +14.0, "pitch_deg": +9.0, "roll_deg": +8.0,
              "centre_z_local_m": -2.80, "radius_m": 0.72, "theta_min_deg": -25.0,
              "theta_max_deg": +25.0, "height_m": 0.18, "texture_tag": 97},
    },
}

# Evaluator-only box-coverage witnesses.  They are design checks only; neither
# component nor ensemble prediction imports or follows them.
DESIGN_WITNESS = {
    "cert_triad_e": {
        201: [(-19.0, +8.0), (-14.0, +8.0), (-9.0, +8.0)],
        202: [( -4.0, -9.0), ( +1.0, -9.0), (+6.0, -9.0)],
        203: [(+11.0, +8.0), (+16.0, +8.0), (+21.0, +8.0)],
    },
    "cert_triad_f": {
        201: [( -4.0, -8.0), ( +1.0, -8.0), (+6.0, -8.0)],
        202: [(-19.0, +9.0), (-14.0, +9.0), (-9.0, +9.0)],
        203: [(+10.0, +9.0), (+15.0, +9.0), (+20.0, +9.0)],
    },
}
...
def spec(fixture: str, object_id: int) -> dict:
    if fixture not in public.FIXTURES or object_id not in public.OBJECT_IDS:
        raise ValueError("unknown Scene-1c fixture/object")
...
def _frame(yaw_deg: float, pitch_deg: float, roll_deg: float) -> np.ndarray:
    ...
    rr = cr*right + sr*up; uu = -sr*right + cr*up
...
def _cylinder_strip(fixture: str, object_id: int, j: int) -> dict:
    ...
    return {"name":f"scene1c_{fixture}_{object_id}_strip_{j:02d}","vertices_h":_to_world(fixture,object_id,local),
...
def scene_objects(fixture: str) -> list[dict]:
    ...
    out.append(quad([0.0,0.0,BACKGROUND_Z_M],[1,0,0],[0,1,0],BACKGROUND_SIZE_M[0],BACKGROUND_SIZE_M[1],
                    public.BACKGROUND_ID,f"scene1c_{fixture}_background"))
    return out


def scene_texture(fixture: str, instance: int, size: int = 512) -> np.ndarray:
    tag=101 if instance==public.BACKGROUND_ID else int(spec(fixture,int(instance))["texture_tag"])
    return texture(int(instance)+191000+211*tag,size)


def truth_points(fixture: str, object_id: int) -> np.ndarray:
    s=spec(fixture,object_id); nt,ny=CYL_GRID_WH
    th=np.radians(np.linspace(s["theta_min_deg"],s["theta_max_deg"],nt)); y=np.linspace(-s["height_m"]/2,s["height_m"]/2,ny)
    T,Y=np.meshgrid(th,y); local=_cylinder_point_local(s,T,Y).reshape(-1,3)
...
def angular_coordinates(xyz_h: np.ndarray) -> tuple[np.ndarray,np.ndarray]:
    ...
    yaw=np.degrees(np.arctan2(p[:,0],-p[:,2])); pitch=np.degrees(np.arctan2(p[:,1],np.sqrt(p[:,0]**2+p[:,2]**2)))
    return yaw,pitch


def angular_bounds(fixture: str, object_id: int) -> tuple[float,float,float,float]:
    y,p=angular_coordinates(truth_points(fixture,object_id)); return float(y.min()),float(y.max()),float(p.min()),float(p.max())
...
def surface_distance(fixture: str, object_id: int, xyz_h: np.ndarray) -> np.ndarray:
    ...
    tc=np.clip(th,math.radians(s["theta_min_deg"]),math.radians(s["theta_max_deg"])); yc=np.clip(p[:,1],-s["height_m"]/2,s["height_m"]/2)
    q=np.c_[s["radius_m"]*np.sin(tc),yc,s["centre_z_local_m"]+s["radius_m"]*np.cos(tc)]
    return np.linalg.norm(p-q,axis=1)


def signed_radial_error(fixture: str, object_id: int, xyz_h: np.ndarray) -> np.ndarray:
    s=spec(fixture,object_id); p=_to_local(fixture,object_id,xyz_h)
    return np.sqrt(p[:,0]**2+(p[:,2]-s["centre_z_local_m"])**2)-s["radius_m"]


def truth_spec(fixture: str) -> dict:
    return {"id":f"Scene1c-{fixture}-truth-v1","public_spec_sha256":public.public_digest(),"fixture":fixture,
            "objects":{str(oid):spec(fixture,oid) for oid in public.OBJECT_IDS},"background_z_h_m":BACKGROUND_Z_M,
            "cylinder_strips":CYLINDER_STRIPS,"truth_cover_radius_m":TRUTH_COVER_RADIUS_M}
...
def validate_mesh(fixture: str, mesh: dict) -> None:
    ...
    if set(ids.tolist()) != set(public.OBJECT_IDS)|{public.BACKGROUND_ID}: raise ValueError("unexpected Scene-1c instance IDs")
    ...
    for oid in public.OBJECT_IDS:
        ...
        if int((ids==oid).sum())!=2*CYLINDER_STRIPS: raise ValueError(f"unexpected triangle count for object {oid}")
        if exp.shape!=got.shape or np.max(np.linalg.norm(exp-got,axis=1))>2e-5: raise ValueError(f"mesh mismatch for object {oid}")
    if int((ids==public.BACKGROUND_ID).sum())!=2: raise ValueError("background triangle count mismatch")
...
def ideal_trace_coverage(fixture: str, object_id: int, gazes: list[tuple[float,float]], fov_deg: float = 12.0) -> float:
    ...
    for gy,gp in gazes: seen|=(np.abs(y-float(gy))<=half)&(np.abs(p-float(gp))<=half)
    ...
    for f in public.FIXTURES:
        ...
        for i,a in enumerate(public.OBJECT_IDS):
            for c in public.OBJECT_IDS[i+1:]:
                A=b[a]; B=b[c]; iw=max(0.0,min(A[1],B[1])-max(A[0],B[0])); ih=max(0.0,min(A[3],B[3])-max(A[2],B[2]))
                if iw*ih>1e-9: raise AssertionError(f"{f} objects {a}/{c} angularly overlap in Scene-1c base case")
        seed_rows={}; seed_map=dict(public.SEED_SEQUENCE[f])
        for oid in public.OBJECT_IDS:
            trace=DESIGN_WITNESS[f][oid]
            if tuple(trace[0])!=tuple(seed_map[oid]): raise AssertionError(f"{f}/{oid} design witness does not start at prescribed seed")
            if len(trace)>=public.PER_OBJECT_MAX_FIXATIONS: raise AssertionError(f"{f}/{oid} design witness leaves no per-object budget slack")
            ...
            if not (0.45<=c0<=0.70): raise AssertionError(f"{f}/{oid} seed is not a controlled partial view: {c0}")
            if cu<0.98: raise AssertionError(f"{f}/{oid} three-look design witness does not span object: {cu}")
...
def self_test() -> None:
    pf=preflight()
    if set(pf)!=set(public.FIXTURES): raise AssertionError("Scene-1c preflight fixture set wrong")
    for f in public.FIXTURES:
        for oid in public.OBJECT_IDS:
            if pf[f]["seeds"][oid]["design_witness_ideal_coverage"]<0.98: raise AssertionError("Scene-1c design witness drifted")
