"""Post-hoc evaluation of one FSG6 3D-frontier active run."""
...
def evaluate(root:Path,out:Path,mode:str)->dict:
    ...
    if fixture not in public.FIXTURES or seed not in public.SEEDS or manifest.get("instrument")!=public.INSTRUMENT_ID: raise ValueError("wrong FSG6 fixture/seed/instrument")
    ...
    metrics["fails"]=fails; metrics["status"]="FSG6_3D_FRONTIER_RUN_PASS" if not fails else "FSG6_3D_FRONTIER_RUN_FAIL"; json_write(out/"metrics.json",metrics); write_visual(out/"growth_truth.png",fixture,maps,cov,gazes); print("[fsg6-eval] "+metrics["status"],json.dumps(metrics,sort_keys=True),flush=True); return metrics
