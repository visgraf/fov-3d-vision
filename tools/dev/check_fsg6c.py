"""Software/integrity checks for FSG6c candidate-aligned 3D surfel-frontier growth."""
...
import fsg6c_scene as scene
import fsg6c_frontier as frontier
import fsg6b_public as old_public
...
def directional_corner_control() -> None:
    q=scene.preflight_transition("corner_up_right",(2.0,3.0),(7.0,8.0))
    if not q["new"]["allowed"]:
        raise AssertionError("candidate-aligned rule rejects the fresh diagonal corner continuation")
    if q["retired_allowed"]:
        raise AssertionError("fresh critical transition no longer distinguishes FSG6c from retired FSG6b conjunction")


def source_isolation_control() -> None:
    root=Path(__file__).resolve().parents[2]
    frontier_src=(root/"tools"/"fsg6c_frontier.py").read_text()
    run_src=(root/"tools"/"fsg6c_run.py").read_text()
    if "fsg6c_scene" in frontier_src or "fsg6c_scene" in run_src:
        raise AssertionError("truth fixture leaked into FSG6c host policy/run")
    if "evaluation_only" in frontier_src or "evaluation_only" in run_src:
        raise AssertionError("evaluation-only truth leaked into FSG6c host policy/run")


def run_positive() -> None:
    passed=0
    if public.FUSION!=frozen_public.FUSION: raise AssertionError("FSG6c fusion differs from frozen 12mm rule")
    if public.SURFACE_FRONTIER!=old_public.SURFACE_FRONTIER: raise AssertionError("FSG6c changed an FSG6b frontier/continuation constant")
    if public.TARGETS!=old_public.TARGETS: raise AssertionError("FSG6c changed an FSG6b numerical gate")
    passed+=1
    scene.self_test(); passed+=1
    frontier.self_test(); passed+=1
    eye_swap_control(); passed+=1
    directional_corner_control(); passed+=1
    ...
    passed+=1
    flat=scene.surface_distance("corner_up_right",flat_chord_points("corner_up_right"))
    ...
    passed+=1
    med=abs(float(np.median(scene.signed_radial_error("corner_up_right",radially_shifted("corner_up_right",-0.015)))))
    ...
    passed+=1
    r=scene.ideal_angular_coverage("corner_up_right",[(-8+5*k,-7) for k in range(5)])
    if r>=0.65: raise AssertionError("horizontal-only control can cover the FSG6c fixture")
    passed+=1
    print(f"[fsg6c-check] SUMMARY passed={passed} failed=0")
...
def run_negative(name: str) -> None:
    ...
    if name=="horizontal":
        c=scene.ideal_angular_coverage("corner_up_right",[(-8+5*k,-7) for k in range(5)])
    ...
    if name=="conjunction":
        q=scene.preflight_transition("corner_up_right",(2.0,3.0),(7.0,8.0))
        if q["new"]["allowed"] and not q["retired_allowed"]:
            raise AssertionError("deliberate retired FSG6b diagonal component-conjunction rule detected")
    if name=="flat":
        if float(np.percentile(scene.surface_distance("corner_up_right",flat_chord_points("corner_up_right")),95))>public.TARGETS["map_surface_p95_max_m"]: raise AssertionError("deliberate flat substitute for curved surface detected")
    if name=="shift":
        if float(np.percentile(scene.surface_distance("corner_up_right",scene.truth_points("corner_up_right")+np.array([0.05,0,0])),95))>public.TARGETS["map_surface_p95_max_m"]: raise AssertionError("deliberate 5cm map shift detected")
    if name=="bias":
        med=abs(float(np.median(scene.signed_radial_error("corner_up_right",radially_shifted("corner_up_right",-0.015)))))
...
def main() -> None:
    ...
    ap.add_argument("--negative",choices=("policy","mapstate","horizontal","monocular","conjunction","flat","shift","bias","purity"))
    a=ap.parse_args()
    try: run_negative(a.negative) if a.negative else run_positive()
    except BaseException as exc:
        print("[fsg6c-check] FAIL",type(exc).__name__,str(exc)); raise SystemExit(1)
