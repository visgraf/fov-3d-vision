"""Blender-side acquisition of one Reality Check 1 fixation pair."""
...
def acquire_one(args: argparse.Namespace) -> dict:
    ...
    if args.step < 0 or args.step >= public.MAX_FIXATIONS:
        raise ValueError("step outside Reality Check 1 budget")
    ...
    acq={"schema":"RealityCheck1-fixation-acquisition-v1","source":backend.source,
         ...
         "scene_character":"mixed-texture nonplanar target with unrelated tabletop clutter",
         "checks":backend.checks}
    json_write(folder/"acquisition.json",acq)
    run={"schema":"RealityCheck1-fixation-run-v1","source":backend.source,"complete":True,
         ...
         "public_spec_sha256":public.public_digest(),"truth_spec_sha256":scene_spec.truth_digest(),
         "total_wall_seconds":time.perf_counter()-t_all}
    json_write(out/"run.json",run)
    print(f"[reality1-render] COMPLETE seed={args.seed} step={args.step} yaw={args.yaw:.3f} pitch={args.pitch:.3f} samples={acq['primary_camera_samples']}",flush=True)
    return run
...
if __name__=="__main__":
    try:
        ...
        if isinstance(exc,SystemExit) and exc.code in (0,None):
            raise
        traceback.print_exc(); print("[reality1-render] FAILED",flush=True)
