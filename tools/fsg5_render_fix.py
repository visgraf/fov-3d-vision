"""Blender-side acquisition of one FSG5 curved-surface fixation pair."""
...
import fsg5_scene as scene_spec
...
def acquire_one(args: argparse.Namespace) -> dict:
    if args.fixture not in public.FIXTURES:
        raise ValueError("fixture not in frozen FSG5 schedule")
    ...
    if args.step < 0 or args.step >= public.MAX_BUDGET_FIXATIONS:
        raise ValueError("step outside frozen FSG5 budget")
    ...
    acq = {
        "schema": "FSG5-fixation-acquisition-v1",
    ...
    print(f"[fsg5-render] COMPLETE fixture={args.fixture} seed={args.seed} step={args.step} yaw={args.yaw:.3f} samples={acq['primary_camera_samples']}", flush=True)
    return run
...
if __name__ == "__main__":
    try:
        ...
        if isinstance(exc, SystemExit) and exc.code in (0, None):
            raise
        traceback.print_exc()
        print("[fsg5-render] FAILED", flush=True)
