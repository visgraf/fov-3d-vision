"""Run one Stage II / Scene-1c seeded multi-object active reconstruction trial."""
...
def save_scene_ply(path:Path,maps:dict[int,SurfaceMap])->None:
    ...
    with path.open("w",encoding="ascii") as f:
        f.write("ply\nformat ascii 1.0\ncomment fixed head frame H; Stage II Scene-1c\n")
...
def write_scene_overview(path:Path,maps:dict[int,SurfaceMap],gazes:list[tuple[float,float]],targets:list[int])->None:
    ...
    cv2.putText(canvas,"Scene-1c final persistent maps and physical fixations",(55,30),cv2.FONT_HERSHEY_SIMPLEX,.60,(20,20,20),1,cv2.LINE_AA)
    cv2.imwrite(str(path),canvas)

def run_blender(args,step:int,target:int,gaze:tuple[float,float],root:Path):
    yaw,pitch=gaze; out=root/f"fix_{step:02d}"
    cmd=[args.blender,"-b","--python-exit-code","1","-P","tools/scene1c_render_fix.py","--","--out",str(out),"--profile",args.profile,"--fixture",args.fixture,"--seed",str(args.seed),"--step",str(step),"--target-object",str(target),"--yaw",f"{yaw:.12g}","--pitch",f"{pitch:.12g}","--device",args.device]
    ...
    if not rr.get("complete") or rr.get("fixture")!=args.fixture or int(rr.get("seed"))!=args.seed or int(rr.get("target_object_id"))!=target or abs(float(rr["yaw_deg"])-yaw)>1e-8 or abs(float(rr["pitch_deg"])-pitch)>1e-8:
        raise RuntimeError("incomplete or wrong Scene-1c Blender record")
    return case,rr
...
def execute(args)->dict:
    ...
    json_write(args.out/"scene_policy_trace.json",{"policy":"frozen Scene-1b least-served-first scheduler over frozen FSG6f object proposals","trace":scheduler_trace})
    final_states={}
    states={oid:_state(oid,maps,histories,last_obs,last_gaze,autonomous_target_counts) for oid in public.OBJECT_IDS}
    ...
    manifest={"schema":"Scene1c-prediction-v1","instrument":public.INSTRUMENT_ID,"public_spec_sha256":public.public_digest(),"profile":args.profile,"fixture":args.fixture,"seed":args.seed,"fixed_head":True,"static_scene":True,
              ...
              "opportunistic_fusion_enabled":True,"global_no_revisit":True,"component_certification_required_by_contract":True,"component_certification_used_as_policy_input":False,"frozen_scene_scheduler":public.FROZEN_SCENE_SCHEDULER_ID,"primary_camera_samples":samples,"blender_recorded_wall_seconds":render_seconds,"loop_wall_seconds":time.perf_counter()-t0}
    json_write(args.out/"prediction_manifest.json",manifest)
    print("[scene1c-run] COMPLETE",json.dumps({"fixture":args.fixture,"seed":args.seed,"fixations":len(global_gazes),"targets":target_sequence,"termination":termination,"samples":samples},sort_keys=True),flush=True)
    return manifest
