"""Blender-side acquisition of one FSG4 fixation pair.
...
import fsg4_scene as scene_spec
...
def acquire_one(args: argparse.Namespace) -> dict:
    if args.fixture not in public.FIXTURES:
        raise ValueError("fixture not in frozen FSG4 schedule")
    ...
    acq = {
        "schema": "FSG4-fixation-acquisition-v1",
    ...
    print(f"[fsg4-render] COMPLETE fixture={args.fixture} seed={args.seed} step={args.step} yaw={args.yaw:.3f} samples={acq['primary_camera_samples']}", flush=True)
    return run
...
if __name__ == "__main__":
    try:
        ...
        if isinstance(exc, SystemExit) and exc.code in (0, None):
            raise
        traceback.print_exc()
        print("[fsg4-render] FAILED", flush=True)
