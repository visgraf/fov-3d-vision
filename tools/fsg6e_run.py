"""Run one FSG6e active trial with the truth-free 3D surfel-frontier policy."""
...
def run_blender(args,step:int,gaze:tuple[float,float],acq_root:Path):
    yaw,pitch=gaze; out=acq_root/f"fix_{step:02d}"; cmd=[args.blender,"-b","--python-exit-code","1","-P","tools/fsg6e_render_fix.py","--","--out",str(out),"--profile",args.profile,"--fixture",args.fixture,"--seed",str(args.seed),"--step",str(step),"--yaw",f"{yaw:.12g}","--pitch",f"{pitch:.12g}","--device",args.device]
    ...
    if not rr.get("complete") or rr.get("fixture")!=args.fixture or int(rr.get("seed"))!=args.seed or abs(float(rr["yaw_deg"])-yaw)>1e-8 or abs(float(rr["pitch_deg"])-pitch)>1e-8: raise RuntimeError("incomplete or wrong FSG6e Blender record")
    return case,rr
...
def execute(args)->dict:
    ...
    if public.FUSION != frozen_public.FUSION:
        raise AssertionError("FSG6e must preserve the frozen 12 mm FSG3/FSG4 fusion rule")
    if args.fixture not in public.FIXTURES or args.seed not in public.SEEDS: raise ValueError("fixture/seed outside frozen FSG6e schedule")
    ...
    for step in range(public.MAX_BUDGET_FIXATIONS):
        ...
        if len(p.xyz_h)<100: raise ValueError("active FSG6e fixation has too few object points")
        if sm is None:
    ...
    save_map(args.out/"surface_map.npz",sm); save_ply(args.out/"surface_map.ply",sm); write_growth(args.out/"growth.png",snapshots,supports,gazes); json_write(args.out/"policy_trace.json",{"policy":"fsg6e_persistent_open_frontier_with_projected_corridor","trace":policy_trace})
    manifest={"schema":"FSG6e-prediction-v1","instrument":public.INSTRUMENT_ID,"public_spec_sha256":public.public_digest(),"profile":args.profile,"fixture":args.fixture,"seed":args.seed,"fixation_gazes_deg":[list(g) for g in gazes],"termination_reason":termination,"patch_stats":patch_stats,"association_stats":assoc_stats,"truth_opened":False,"policy_inputs":["persistent map xyz_h","current rectified oracle instance masks L/R","raw calibration support L/R","calibration","fixation history","completed binocular oracle/support history"],"primary_camera_samples":samples,"blender_recorded_wall_seconds":render_seconds,"loop_wall_seconds":time.perf_counter()-t0}
    json_write(args.out/"prediction_manifest.json",manifest); print("[fsg6e-run] COMPLETE",json.dumps({"fixture":args.fixture,"seed":args.seed,"fixations":len(gazes),"gazes":gazes,"termination":termination,"samples":samples},sort_keys=True),flush=True); return manifest
