"""FSG1h corrected final prospective validation specification.
...
SPEC_ID = "FSG1h-corrected-final-one-step-validation-v1"
...
SPEC = {
    "id": SPEC_ID,
    "supersedes_invalid_fixture": "D-FSG1g smoke integrity failure: occluder edges outside accepted core",
    "candidate": "FSG1-HDR-SGBM-one-original-update-original-validity-v1",
    "cases": list(CASES),
    ...
    "occluder_left": {
        ...
        "foreground_x_interval_m": [-0.16, 0.10],
    ...
    "occluder_right": {
        ...
        "foreground_x_interval_m": [-0.10, 0.16],
        "foreground_height_m": 2.2,
        "background_size_m": [4.6, 4.2],
...
def final_objects(name: str) -> list[dict]:
    ...
    raise ValueError("unknown FSG1h case: " + str(name))
...
def validate_mesh(name: str, mesh: dict) -> None:
    ...
    for obj in wanted:
        ...
        if np.max(np.min(distance, axis=-1)) > 2e-5:
            raise ValueError("exported Blender mesh disagrees with frozen FSG1h specification")
...
def self_test() -> None:
    ...
    print("[fsg-finalh-scene] PASS cases=6 phases=4 corrected_finite_occluders=2")
