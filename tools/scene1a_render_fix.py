"""Blender-side acquisition of one Stage II / Scene-1a binocular fixation."""
from __future__ import annotations
import argparse, datetime as dt, sys, time, traceback
from pathlib import Path
import numpy as np
sys.path.insert(0,str(Path(__file__).resolve().parent))
import fsg_validation_render as base
import scene1a_public as public
import scene1a_scene as scene_spec
...
def acquire_one(args:argparse.Namespace)->dict:
    ...
    if args.step<0 or args.step>=public.MAX_SCENE_FIXATIONS: raise ValueError("step outside Scene-1a budget")
    if args.target_object not in public.OBJECT_IDS: raise ValueError("unknown target object")
    ...
    acq={"schema":"Scene1a-fixation-acquisition-v1","source":backend.source,"case":case,"step":args.step,"fixture":args.fixture,"profile":args.profile,
         "target_object_id":int(args.target_object),"yaw_deg":args.yaw,"pitch_deg":args.pitch,"spp":spp,"seeds_lr":seeds,"blender_version":backend.version,"device":backend.device,
    ...
    run={"schema":"Scene1a-fixation-run-v1","source":backend.source,"complete":True,"created_utc":dt.datetime.now(dt.timezone.utc).isoformat(),
         "case":case,"step":args.step,"fixture":args.fixture,"profile":args.profile,"seed":args.seed,"target_object_id":int(args.target_object),"yaw_deg":args.yaw,"pitch_deg":args.pitch,"spp":spp,
         "seeds_lr":seeds,"primary_camera_samples":acq["primary_camera_samples"],"public_spec_sha256":public.public_digest(),"truth_spec_sha256":scene_spec.truth_digest(args.fixture),"total_wall_seconds":time.perf_counter()-t_all}
    json_write(out/"run.json",run)
    print(f"[scene1a-render] COMPLETE fixture={args.fixture} seed={args.seed} step={args.step} target={args.target_object} yaw={args.yaw:.3f} pitch={args.pitch:.3f} samples={acq['primary_camera_samples']}",flush=True)
    return run
...
def parse_args(argv:list[str])->argparse.Namespace:
    ...
    ap.add_argument("--target-object",type=int,choices=public.OBJECT_IDS,required=True); ap.add_argument("--yaw",type=float,required=True); ap.add_argument("--pitch",type=float,required=True)
    ap.add_argument("--device",choices=("OPTIX","CUDA","CPU"),default="OPTIX"); ap.add_argument("--save-blend",action="store_true"); return ap.parse_args(argv)


def main()->None:
    try:
        acquire_one(parse_args(sys.argv[sys.argv.index("--")+1:] if "--" in sys.argv else sys.argv[1:]))
    except Exception:
