"""Post-hoc evaluation of one FSG6f 3D-frontier active run."""
...
def evaluate(root:Path,out:Path,mode:str)->dict:
    ...
    if fixture not in public.FIXTURES or seed not in public.SEEDS or manifest.get("instrument")!=public.INSTRUMENT_ID: raise ValueError("wrong FSG6f fixture/seed/instrument")
    ...
    for i,d in enumerate(trace[:-1]):
        if not d.get("stop",False):
            ...
            con=sel.get("candidate_state_consensus") or {}
            if not bool(con.get("allowed",False)) or int(sel.get("frontier_support_count",0))<=int(sel.get("resolved_frontier_support_count",10**9)):
                fails.append(f"policy decision {i} lacks OPEN-majority candidate consensus")
    ...
    metrics["fails"]=fails; metrics["status"]="FSG6F_3D_FRONTIER_RUN_PASS" if not fails else "FSG6F_3D_FRONTIER_RUN_FAIL"; json_write(out/"metrics.json",metrics); write_visual(out/"growth_truth.png",fixture,maps,cov,gazes); print("[fsg6f-eval] "+metrics["status"],json.dumps(metrics,sort_keys=True),flush=True); return metrics
