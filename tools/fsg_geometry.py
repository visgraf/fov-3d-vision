...
def camera_rotation_h(direction: np.ndarray) -> np.ndarray:
    """OpenCV-to-head rotation, zero torsion; equals the repository rig convention."""
    z = unit(direction)
...
def make_calibration(profile: str, yaw: float = 0., pitch: float = 0.,
                     ...
                     head_origin_w: np.ndarray = HEAD_ORIGIN_W) -> dict[str, Any]:
    ...
    for name, x in (("L", -ipd/2), ("R", ipd/2)):
        ...
        eyes.append({"name": name, "centre_h_m": centre.tolist(),
                     "R_hc": camera_rotation_h(target-centre).tolist(), "K": k.tolist()})
