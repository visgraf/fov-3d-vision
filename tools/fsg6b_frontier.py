"""Truth-free 3D surfel-frontier controller for FSG6b.
...
def angular_coordinates(xyz_h: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    p = p[good]
    if len(p) < 100:
        raise ValueError("too few finite forward map points for FSG6b policy")
...
def _candidate_edge_allowed(dx: int, dy: int, ev: dict) -> bool:
    # Every nonzero movement component must still contain object at the
    # corresponding current-image edge.  This prevents a diagonal move from
    # crossing an already-resolved physical boundary in its other component.
    ok = []
    ...
    for dx in (-1,0,1):
        for dy in (-1,0,1):
            ...
            if len(seen) and np.any(np.all(np.isclose(seen, [yaw,pitch], atol=1e-9), axis=1)):
                continue
            if not _candidate_edge_allowed(dx, dy, ev):
                continue
...
def self_test() -> None:
    ...
    # Eye-swap invariance: only one eye carries top/right continuation. Swapping eyes
    # must not change the binocular evidence or the selected 3D-frontier gaze.
    ...
    m=_xyz_patch((-11,1),(-11,1))
    c1=choose_next(0.0,0.0,cal,L,sup,R,sup,m,[(0.0,0.0)])
    c2=choose_next(0.0,0.0,cal,R,sup,L,sup,m,[(0.0,0.0)])
    if c1["next_gaze_deg"] != c2["next_gaze_deg"]:
        raise AssertionError("selected gaze changed under L/R swap")
    # Demonstrate why the retired left-eye-only rule is not invariant.
    old1=edge_evidence(L,sup,public.OBJECT_ID); old2=edge_evidence(R,sup,public.OBJECT_ID)
    if old1["touch_right"] == old2["touch_right"] and old1["touch_top"] == old2["touch_top"]:
        raise AssertionError("synthetic eye-swap test does not exercise monocular asymmetry")
    # Resolved physical boundary in BOTH eyes -> stop even with geometric frontier.
    ...
    print("[fsg6b-frontier] PASS map_state_changes_2d_direction=true eye_swap_invariant=true resolved_boundary_stops=true")
