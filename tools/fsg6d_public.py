"""Truth-free public contract for FSG6d projected-frontier corridor validation.

FSG6b made the segmentation veto binocular but its diagonal component conjunction
was too strict. FSG6c replaced conjunction with max(component edges), which was
too permissive: one strong component could license motion across a fully resolved
orthogonal boundary. FSG6d removes full-edge Boolean composition altogether.
For each 3D-frontier-supported candidate, the supporting surfels and their 3D
look-ahead targets are projected into both rectified cores; segmentation is read
only in their candidate-local forward exit corridors. No numerical policy
threshold, frontier constant, instrument, fusion rule, lattice, budget or metric
gate is changed.
"""
...
SPEC_ID = "FSG6d-projected-frontier-exit-corridor-v1"
INSTRUMENT_ID = "FSG1-HDR-SGBM-one-original-update-original-validity-v1"
OBJECT_ID = 121
BACKGROUND_ID = 122
FIXTURES = ("corridor_up_right", "corridor_down_left")
SEEDS = (1009, 1061)
DEFAULT_SPP = {"small": 64, "full": 256}
SEED_GAZE_DEG = {
    "corridor_up_right": (-8.0, -7.0),
    "corridor_down_left": (+8.0, +7.0),
}
...
# Byte-for-byte numerical values inherited from FSG6a/b/c. Do not tune in FSG6d.
SURFACE_FRONTIER = {
...
PUBLIC_SPEC = {
    ...
    "continuation_veto": "binocular projected 3D-frontier exit corridor in the candidate forward image sector",
    "continuation_threshold_unchanged_from_fsg6c": True,
    "legacy_edge_band_fraction_reused_as_corridor_width": True,
    "retired_fsg6b_component_conjunction": True,
    "retired_fsg6c_component_max": True,
    "no_icp": True,
    "no_hole_fill": True,
...
def fixture_index(fixture: str) -> int:
    if fixture not in FIXTURES:
        raise ValueError(f"unknown FSG6d fixture {fixture}")
...
def render_seed(fixture: str, seed: int, yaw_deg: float, pitch_deg: float, eye_id: int) -> int:
    ...
    if seed not in SEEDS:
        raise ValueError("seed not in frozen FSG6d schedule")
    ...
    if not (p["yaw_min_deg"] - 1e-9 <= y <= p["yaw_max_deg"] + 1e-9):
        raise ValueError("yaw outside frozen FSG6d range")
    if not (p["pitch_min_deg"] - 1e-9 <= q <= p["pitch_max_deg"] + 1e-9):
        raise ValueError("pitch outside frozen FSG6d range")
