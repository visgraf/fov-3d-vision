"""Truth-free public contract for FSG6c candidate-aligned 3D surface-frontier validation.

FSG6b repaired the reference-eye asymmetry of the auxiliary segmentation veto,
but preserved an axis-separable diagonal rule: a diagonal candidate required
continuation on BOTH component edges. A rolled ribbon can legitimately leave a
foveal core through the forward perimeter without touching both component edges.
FSG6c keeps the 3D surfel frontier, stereo instrument, fusion rule, lattice,
thresholds and numerical gates fixed; it changes only that veto semantics.
"""
from __future__ import annotations
import hashlib
import json

SPEC_ID = "FSG6c-directional-perimeter-surface-frontier-v1"
INSTRUMENT_ID = "FSG1-HDR-SGBM-one-original-update-original-validity-v1"
OBJECT_ID = 111
BACKGROUND_ID = 112
FIXTURES = ("corner_up_right", "corner_down_left")
SEEDS = (907, 953)
DEFAULT_SPP = {"small": 64, "full": 256}
SEED_GAZE_DEG = {
    "corner_up_right": (-8.0, -7.0),
    "corner_down_left": (+8.0, +7.0),
}
...
# Numerically identical to FSG6a/FSG6b. Do not tune these constants in FSG6c.
SURFACE_FRONTIER = {
...
PUBLIC_SPEC = {
    ...
    "binocular_continuation_veto": "candidate-aligned forward-perimeter evidence: max over L/R eyes and direction-compatible component edges",
    "continuation_threshold_unchanged_from_fsg6b": True,
    "diagonal_component_conjunction_retired": True,
    "no_icp": True,
    "no_hole_fill": True,
...
def fixture_index(fixture: str) -> int:
    if fixture not in FIXTURES:
        raise ValueError(f"unknown FSG6c fixture {fixture}")
...
def render_seed(fixture: str, seed: int, yaw_deg: float, pitch_deg: float, eye_id: int) -> int:
    ...
    if seed not in SEEDS:
        raise ValueError("seed not in frozen FSG6c schedule")
    ...
    if not (p["yaw_min_deg"] - 1e-9 <= y <= p["yaw_max_deg"] + 1e-9):
        raise ValueError("yaw outside frozen FSG6c range")
    if not (p["pitch_min_deg"] - 1e-9 <= q <= p["pitch_max_deg"] + 1e-9):
        raise ValueError("pitch outside frozen FSG6c range")
