"""Blender-side acquisition of one Stage II / Scene-1b binocular fixation."""
...
import scene1b_scene as scene_spec
...
def acquire_one(args:argparse.Namespace)->dict:
    ...
    if args.step<0 or args.step>=public.MAX_SCENE_FIXATIONS: raise ValueError("step outside Scene-1b budget")
    ...
    acq={"schema":"Scene1b-fixation-acquisition-v1","source":backend.source,"case":case,"step":args.step,"fixture":args.fixture,"profile":args.profile,
    ...
    run={"schema":"Scene1b-fixation-run-v1","source":backend.source,"complete":True,"created_utc":dt.datetime.now(dt.timezone.utc).isoformat(),
    ...
    print(f"[scene1b-render] COMPLETE fixture={args.fixture} seed={args.seed} step={args.step} target={args.target_object} yaw={args.yaw:.3f} pitch={args.pitch:.3f} samples={acq['primary_camera_samples']}",flush=True)
    return run
