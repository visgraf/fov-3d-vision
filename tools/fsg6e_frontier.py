"""Truth-free 3D surfel-frontier controller for FSG6e.

FSG6e keeps the FSG6d raw 3D frontier extraction, candidate-local binocular
continuation corridor and ranking, but adds persistent frontier state.  A raw
frontier whose existing 3D look-ahead target is already represented by the
persistent map within the frozen FSG3 association radius is MAP_RESOLVED and
cannot support a new gaze.  The remaining OPEN frontiers are projected into
both rectified cores and evaluated by the unchanged FSG6d corridor.  No truth
is used by this module and no new numerical threshold is introduced.
"""
...
def angular_coordinates(xyz_h: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    p = p[good]
    if len(p) < 100:
        raise ValueError("too few finite forward map points for FSG6e policy")
...
def self_test() -> None:
    ...
    print("[fsg6e-frontier] PASS map_state_changes_2d_direction=true persistent_state_three_way=true historical_boundary_state=true stereo_hole_not_boundary=true eye_swap_invariant=true projected_frontier_corridor=true resolved_boundary_stops=true")
