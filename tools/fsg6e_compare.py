"""Aggregate the four prospectively fixed full FSG6e 3D-frontier trials."""
...
def compare(metric_files:list[Path],out:Path)->dict:
    ...
    for p in metric_files:
        ...
        if key in seen: raise ValueError("duplicate FSG6e trial")
        seen.add(key); rows.append(r)
    if seen!=EXPECTED: raise ValueError(f"FSG6e full set incomplete: got {sorted(seen)} expected {sorted(EXPECTED)}")
    rows.sort(key=lambda r:(r["fixture"],r["seed"])); fails=[]
    for r in rows:
        if r["profile"]!="full": fails.append(f"{r['fixture']}/{r['seed']} is not full profile")
        if r["status"]!="FSG6E_3D_FRONTIER_RUN_PASS" or r["fails"]: fails.append(f"{r['fixture']}/{r['seed']} 3D-frontier run failed")
    result={"schema":"FSG6e-comparison-v1","trials":rows,"trial_passes":sum(not r["fails"] for r in rows),"mean_final_coverage":float(np.mean([r["truth_coverage_final"] for r in rows])),"pitch_span_range_deg":[float(np.min([r["pitch_span_deg"] for r in rows])),float(np.max([r["pitch_span_deg"] for r in rows]))],"surface_median_range_m":[float(np.min([r["map_surface_median_m"] for r in rows])),float(np.max([r["map_surface_median_m"] for r in rows]))],"surface_p95_range_m":[float(np.min([r["map_surface_p95_m"] for r in rows])),float(np.max([r["map_surface_p95_m"] for r in rows]))],"supported_signed_radial_median_range_m":[float(np.min([r["supported_signed_radial_median_m"] for r in rows])),float(np.max([r["supported_signed_radial_median_m"] for r in rows]))],"fails":fails,"status":"FSG6E_INCREMENT6_PASS" if not fails else "FSG6E_INCREMENT6_FAIL"}
    out.mkdir(parents=True); json_write(out/"comparison.json",result); write_visual(out/"coverage_3d_frontier.png",rows); print("[fsg6e-compare] "+result["status"],json.dumps(result,sort_keys=True),flush=True); return result
