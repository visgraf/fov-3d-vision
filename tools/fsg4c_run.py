"""Run one FSG4c policy on one opaque fixture/seed pair.
...
def run_blender(args, step: int, yaw: float, acq_root: Path) -> tuple[Path, dict]:
    ...
    cmd = [args.blender, "-b", "--python-exit-code", "1", "-P", "tools/fsg4c_render_fix.py", "--",
...
def execute(args) -> dict:
    ...
    if public.POLICY != frozen_public.POLICY or public.OBJECT_ID != frozen_public.OBJECT_ID or public.BACKGROUND_ID != frozen_public.BACKGROUND_ID:
        raise AssertionError("FSG4c must use the frozen FSG4 frontier policy constants and object IDs")
    if tuple(public.SCAN_YAWS_DEG) != tuple(frozen_public.SCAN_YAWS_DEG):
        raise AssertionError("FSG4c fixed scan differs from FSG4a/b")
    ...
    if args.policy_name not in ("active", "scan"):
        raise ValueError("unknown FSG4c policy")
    ...
    manifest = {
        "schema":"FSG4c-prediction-v1","instrument":public.INSTRUMENT_ID,
    ...
    print("[fsg4c-run] COMPLETE", json.dumps({"fixture":args.fixture,"seed":args.seed,"policy":args.policy_name,"fixations":len(yaws),"yaws":yaws,"termination":termination,"samples":samples,"new_samples":new_samples,"reused_views":reused_views}, sort_keys=True), flush=True)
    return manifest
