...
class PairRenderer:
    """One Blender session that renders verged pairs on demand (C2, D17): `main()` below feeds
    it a target list, `active_loop.py` feeds it one pair at a time from a policy. Everything
    `main()` wrote before the refactor is written by the same code here; `check_pairs.py`,
    `stereo_truth.py`, `stereo_field.py` read the result unchanged."""

    def __init__(self, args, out: str):
        import bpy
        from bl_common import ensure_cycles, eye_record, find_eye, rigid, setup_device
        from render_foveated import render_fixation, setup_foveated_camera
        self.args, self.out = args, os.path.abspath(out)
        self.verged = args.vergence == "on"
        for e in EYE_NAMES:
            os.makedirs(os.path.join(self.out, e), exist_ok=True)
        self.t_seq0 = time.perf_counter()
        self.scene = bpy.context.scene
        ensure_cycles(self.scene)
        self.backend = setup_device(self.scene, args.device)
        bpy.context.view_layer.update()
        self.eye, self.eye_note = find_eye(self.scene)
        eye_m = rigid(self.eye.matrix_world)
        self.head_rot3 = np.array([[eye_m[r][c] for c in range(3)] for r in range(3)], dtype=np.float64)
        self.head_origin = np.array([eye_m.translation[k] for k in range(3)], dtype=np.float64)
        self.centres = eye_centres(self.head_origin, self.head_rot3, args.ipd)
        self.offsets = eye_offsets_local(args.ipd)
        self.n = args.n if args.n else raster_size(args.s0, args.e2, args.emax)
        self.s0 = s0_of(self.n, args.e2, args.emax)
        self.rs = raster_samples(self.n, args.e2, args.emax)
        self.inside = self.rs["inside"]
        self.n_inside = int(self.inside.sum())
        self.cp = centre_pixels(self.n)
        self.cam = setup_foveated_camera(self.scene, self.eye, args.shader, args.e2, args.emax, self.n, args.spp, exr_codec="NONE")
        self.seq_fix = {e: [] for e in EYE_NAMES}
        self.warmup = None
        self.eye_record = eye_record
        self.blend = bpy.data.filepath
        self.blender = bpy.app.version_string

    def banner(self, n_pairs: int):
        a = self.args
        print(f"[pairs] {self.eye_note}; device {self.backend}; raster {self.n}x{self.n}; s0 {self.s0:.4f} deg; E2 {a.e2}; e_max {a.emax}; "
              f"ipd {a.ipd} m; vergence {a.vergence}; {n_pairs} pairs at {a.spp} spp; "
              f"{self.n_inside} samples per fixation", flush=True)
        print(f"[pairs] head origin {self.head_origin.round(4).tolist()}; eye L {self.centres[0].round(4).tolist()} "
              f"R {self.centres[1].round(4).tolist()}", flush=True)

    def pair_for(self, pt: dict) -> dict:
        """A pair record for a fixation point dict (name, kind, point_m, target)."""
        return dict(pt, **pair_for_point(np.array(pt["point_m"]), self.head_origin, self.head_rot3, self.args.ipd, self.verged))

    def warm_up(self, g0: dict):
        from render_foveated import render_fixation, set_gaze
        set_gaze(self.cam, self.eye, g0["yaw"], g0["pitch"], self.offsets[0])
        self.warmup = render_fixation(self.scene, self.args.spp)
        print(f"[pairs] warm-up render {self.warmup:.3f}s (discarded)", flush=True)

    def grab(self, pid: int, k: int, g: dict, point_m: list, exr_path: str) -> tuple[dict, dict]:
        ...
        from render_foveated import save_render_result
        n, rs, inside = self.n, self.rs, self.inside
        save_render_result(self.scene, exr_path)             # before the next render: shared buffer
        ...
        rec = {
            "origin": self.centres[k].astype(np.float32),
            ...
            "fixation_id": np.full(self.n_inside, pid, dtype=np.int32),
            "raster_index": rs["raster_index"][inside].astype(np.int32),
            "eye_id": np.full(self.n_inside, k, dtype=np.int32),
            "pair_id": np.full(self.n_inside, pid, dtype=np.int32),
        }
        cp = self.cp
        c_pos = pos[cp[:, 0], cp[:, 1]]
    ...
    def render_pair(self, pid: int, g_pair: dict, quiet: bool = False) -> dict:
        """Render both eyes of one pair, write f<NNN>/ for each, return {'L': rec, 'R': rec}
        (the in-memory D1 records, so a caller in the session need not read them back)."""
        from render_foveated import fixation_meta, render_fixation, set_gaze
        a = self.args
        g_pair["render"] = {}
        recs = {}
        ...
        for k, g in enumerate(g_pair["eyes"]):
            en = EYE_NAMES[k]
            fdir = os.path.join(self.out, en, f"f{pid:03d}")
            os.makedirs(fdir, exist_ok=True)
            set_gaze(self.cam, self.eye, g["yaw"], g["pitch"], self.offsets[k])
            secs = render_fixation(self.scene, a.spp, seed=EYE_SEEDS[k][0])
            t_io = time.perf_counter()
            rec, measured = self.grab(pid, k, g, g_pair["point_m"], os.path.join(fdir, "fix.exr"))
            np.savez(os.path.join(fdir, "samples.npz"), **rec)
            meta = fixation_meta(self.scene, self.eye, self.eye_note, self.cam, self.backend, self.n, self.s0, a.e2, a.emax, a.spp,
                                 g["yaw"], g["pitch"], secs, a.profile, seconds_include_write=False)
            meta.update({"schema": SCHEMA, "fixation_id": pid, "pair_id": pid, "eye": en, "eye_id": k,
                         "eye_centre_m": self.centres[k].round(6).tolist(), "eye_offset_local_m": self.offsets[k].tolist(),
                         "ipd_m": a.ipd, "vergence": a.vergence, "point_m": g_pair["point_m"],
                         "name": g_pair["name"], "target_kind": g_pair["kind"],
                         "samples_inside_disc": self.n_inside, "exr_codec": "NONE", "seed": EYE_SEEDS[k][0],
                         "seed_b": EYE_SEEDS[k][1] if a.seed_pair else None,
                         "footprint_sum_sr": float(rec["footprint"].sum()), "cap_sr": cap_sr(a.emax),
            ...
            self.seq_fix[en].append({"id": pid, "name": g_pair["name"], "yaw": g["yaw"], "pitch": g["pitch"],
                                     "render_seconds": round(secs, 4), "io_seconds": round(io_secs, 4),
                                     "samples": self.n_inside, "hits": measured["hits"], "pair_id": pid, "eye": en})
    ...
    def seed_pass(self, pairs: list[dict]):
        from render_foveated import render_fixation, set_gaze
        t_b0 = time.perf_counter()
        for pid, g_pair in enumerate(pairs):
            for k, g in enumerate(g_pair["eyes"]):
                fdir = os.path.join(self.out, EYE_NAMES[k], f"f{pid:03d}")
                set_gaze(self.cam, self.eye, g["yaw"], g["pitch"], self.offsets[k])
                secs_b = render_fixation(self.scene, self.args.spp, seed=EYE_SEEDS[k][1])
                rec_b, _ = self.grab(pid, k, g, g_pair["point_m"], os.path.join(fdir, "fix_b.exr"))
                np.savez(os.path.join(fdir, "samples_b.npz"), **rec_b)
                self.seq_fix[EYE_NAMES[k]][pid]["render_seconds_seed_b"] = round(secs_b, 4)
        render_fixation(self.scene, self.args.spp, seed=0)
        print(f"[pairs] seed-pair pass: {2 * len(pairs)} renders in {time.perf_counter() - t_b0:.1f}s", flush=True)

    def finish(self, pairs: list[dict], extra: dict | None = None) -> dict:
        """pairs.json and the per-eye sequence.json; `extra` is merged into pairs.json (the loop
        adds its policy record there)."""
        a = self.args
        total = time.perf_counter() - self.t_seq0
        common = {"blend": self.blend, "device": self.backend, "blender": self.blender,
                  "profile": a.profile, "eye_note": self.eye_note, "schema": SCHEMA,
                  "warp": {"E2_deg": a.e2, "e_max_deg": a.emax, "s0_deg": self.s0, "raster": self.n},
                  "spp": a.spp, "samples_per_fixation": self.n_inside, "seed_pair": a.seed_pair,
                  "seeds_per_eye": {EYE_NAMES[k]: list(EYE_SEEDS[k]) for k in range(2)},
                  "seeds_note": SEEDS_NOTE,
                  "warmup_seconds_discarded": self.warmup}
        rig = {"head": self.eye_record(self.eye), "head_origin_m": self.head_origin.tolist(),
               "head_rot3_world_from_local": self.head_rot3.tolist(), "ipd_m": a.ipd,
               "eye_offsets_local_m": self.offsets.tolist(), "eye_centres_m": self.centres.tolist(),
        ...
        for k, en in enumerate(EYE_NAMES):
            rsec = np.array([f["render_seconds"] for f in self.seq_fix[en]])
            seq = dict(common)
            seq.update({"eye": en, "eye_id": k, "origin_m": self.centres[k].tolist(), "rig": rig,
                        "vergence": a.vergence,
                        "gazes": [{"name": p["name"], "yaw": p["eyes"][k]["yaw"], "pitch": p["eyes"][k]["pitch"],
                                   "kind": p["kind"], "pair_id": i} for i, p in enumerate(pairs)],
                        "fixations": self.seq_fix[en],
            ...
            with open(os.path.join(self.out, en, "sequence.json"), "w") as fh:
                json.dump(seq, fh, indent=1)
        all_sec = np.array([g["render_seconds"] for p in pairs for g in p["eyes"]])
        result = dict(common)
        result.update({"rig": rig, "vergence": a.vergence, "verged": self.verged,
                       "targets_file": os.path.abspath(a.targets) if getattr(a, "targets", None) else None,
        ...
        if extra:
            result.update(extra)
        with open(os.path.join(self.out, "pairs.json"), "w") as fh:
        ...
        print(f"[pairs] {len(pairs)} pairs: {result['pair_seconds_median']:.3f}s median per pair, sum {all_sec.sum():.1f}s, "
              ...
              f"max {1e3 * miss.max() if len(miss) else float('nan'):.2f} mm (in-session; judged by check_pairs.py) -> {self.out}",
              flush=True)
        return result


def add_render_args(ap):
    """The render arguments shared by main() and active_loop.py."""
    from render_foveated import HERE
...
def main():
    ...
    points = load_points(args.targets, args.points, R.head_origin)
    pairs = [R.pair_for(pt) for pt in points]
    R.banner(len(pairs))
    R.warm_up(pairs[0]["eyes"][0])
    for pid, g_pair in enumerate(pairs):
        R.render_pair(pid, g_pair)
    if args.seed_pair:
        R.seed_pass(pairs)
    R.finish(pairs)
