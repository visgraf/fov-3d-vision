"""Run one FSG6d active trial with the truth-free 3D surfel-frontier policy."""
...
def run_blender(args,step:int,gaze:tuple[float,float],acq_root:Path):
    yaw,pitch=gaze; out=acq_root/f"fix_{step:02d}"; cmd=[args.blender,"-b","--python-exit-code","1","-P","tools/fsg6d_render_fix.py","--","--out",str(out),"--profile",args.profile,"--fixture",args.fixture,"--seed",str(args.seed),"--step",str(step),"--yaw",f"{yaw:.12g}","--pitch",f"{pitch:.12g}","--device",args.device]
    ...
    if not rr.get("complete") or rr.get("fixture")!=args.fixture or int(rr.get("seed"))!=args.seed or abs(float(rr["yaw_deg"])-yaw)>1e-8 or abs(float(rr["pitch_deg"])-pitch)>1e-8: raise RuntimeError("incomplete or wrong FSG6d Blender record")
    return case,rr
...
def execute(args)->dict:
    ...
    if public.FUSION != frozen_public.FUSION:
        raise AssertionError("FSG6d must preserve the frozen 12 mm FSG3/FSG4 fusion rule")
    if args.fixture not in public.FIXTURES or args.seed not in public.SEEDS: raise ValueError("fixture/seed outside frozen FSG6d schedule")
    ...
    for step in range(public.MAX_BUDGET_FIXATIONS):
        ...
        if len(p.xyz_h)<100: raise ValueError("active FSG6d fixation has too few object points")
        if sm is None:
