"""Post-hoc evaluation of one FSG4 active or fixed-scan run."""
...
def truth_points(fixture: str) -> np.ndarray:
    ...
    xs = np.linspace(-scene.OBJECT_WIDTH_M/2, scene.OBJECT_WIDTH_M/2, nx)
    ys = np.linspace(-scene.OBJECT_HEIGHT_M/2, scene.OBJECT_HEIGHT_M/2, ny)
    X, Y = np.meshgrid(xs, ys)
    r, u, _ = scene.object_axes()
    c = scene.object_centre(fixture)
    return (c + X[...,None]*r + Y[...,None]*u).reshape(-1,3)


def spatial_coverage(fixture: str, xyz: np.ndarray) -> float:
    truth = truth_points(fixture)
...
def plane_errors(fixture: str, xyz: np.ndarray) -> np.ndarray:
    _, _, w = scene.surface_coordinates(fixture, np.asarray(xyz,float))
    return np.abs(w)
...
def write_visual(path: Path, fixture: str, maps: list, coverage: list[float], yaws: list[float], label: str) -> None:
    ...
    for k,(m,cov,yaw) in enumerate(zip(maps,coverage,yaws)):
        ...
        px=np.clip(((x/scene.OBJECT_WIDTH_M)+.5)*240+x0+30,x0+30,x0+270).astype(int)
        py=np.clip((.5-y/scene.OBJECT_HEIGHT_M)*210+60,60,270).astype(int)
...
def evaluate(root: Path, out: Path, mode: str) -> dict:
    ...
    if fixture not in public.FIXTURES or seed not in public.SEEDS or pol not in ("active","scan"):
        raise ValueError("wrong FSG4 fixture/seed/policy")
    ...
    for step in range(n):
        ...
        if rr.get("truth_spec_sha256")!=scene.truth_digest(fixture) or rr.get("fixture")!=fixture or int(rr.get("seed"))!=seed or abs(float(rr["yaw_deg"])-yaws[step])>1e-8:
    ...
    final=maps[-1];pe=plane_errors(fixture,final.xyz_h)
    patch=manifest["patch_stats"];assoc=manifest["association_stats"]
    metrics={
        "schema":"FSG4-run-evaluation-v1","profile":manifest["profile"],"fixture":fixture,"seed":seed,
        "policy":pol,"instrument":manifest["instrument"],"fixation_count":n,"fixation_yaws_deg":yaws,
        "termination_reason":manifest["termination_reason"],"patch_stats":patch,"association_stats":assoc,
        "truth_coverage_by_fixation":cov,"truth_coverage_gain_by_fixation":gains,
        "truth_coverage_seed":cov[0],"truth_coverage_final":cov[-1],"truth_coverage_gain_over_seed":cov[-1]-cov[0],
        "map_points":int(len(final.xyz_h)),"map_support_histogram":{str(int(k)):int(v) for k,v in zip(*np.unique(final.support_count,return_counts=True))},
    ...
    # Common geometric integrity: both policies must build a metrically valid, pure map.
    ...
    if pol=="active":
        ...
        if cov[-1]-cov[0]<t["active_truth_coverage_gain_over_seed_min"]:fails.append("active insufficient coverage growth over seed")
        for i in range(1,n):
            min_gain=t["minimum_incremental_coverage_gain_terminal"] if i==n-1 else t["minimum_incremental_coverage_gain_nonterminal"]
            if gains[i]<min_gain:fails.append(f"active fixation {i} added too little visible surface")
    else:
    ...
    metrics["fails"]=fails
    metrics["status"]=("FSG4_ACTIVE_RUN_PASS" if pol=="active" else "FSG4_SCAN_RUN_PASS") if not fails else ("FSG4_ACTIVE_RUN_FAIL" if pol=="active" else "FSG4_SCAN_RUN_FAIL")
    json_write(out/"metrics.json",metrics)
    write_visual(out/"growth_truth.png",fixture,maps,cov,yaws,pol)
    print("[fsg4-eval] "+metrics["status"],json.dumps(metrics,sort_keys=True),flush=True)
    return metrics
