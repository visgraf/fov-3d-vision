"""Structural comparator for completed MultiObject-3a next-object selections."""
...
def main() -> None:
    ...
    for p in map(Path, a.records):
        ...
        if m.get("new_object_instantiated") is not False or not m.get("scene_objects_read_only"):
            fails.append(f"{p}: scene object state changed during selection")
        instantiated = {int(x) for x in m.get("instantiated_object_ids", [])}
        selected = m.get("selected_object_id")
        if selected is not None and int(selected) in instantiated:
    ...
    status = "MULTIOBJECT3A_COMPLETE" if not fails else "MULTIOBJECT3A_INTEGRITY_FAIL"
    print("[multiobject3a-compare]", status, json.dumps({"records": rows, "structural_fails": fails}, sort_keys=True))
