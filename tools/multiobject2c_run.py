"""Grow the MultiObject-2b selected-object seed with frozen FSG6f exploration."""
...
import multiobject2b_public as parent_public
from multiobject2b_run import _shared_chart, _write_scene_png, _overlap_counts
import multiobject2c_policy as policy
import multiobject2c_public as public
...
def _validate_parent(parent: Path) -> tuple[dict, dict, int, dict[int, dict]]:
    m = json.loads((parent / "prediction_manifest.json").read_text())
    if m.get("schema") != public.PARENT_SPEC_ID:
        raise AssertionError("MultiObject-2c requires a completed MultiObject-2b parent")
    if m.get("public_spec_sha256") != parent_public.public_digest():
        raise AssertionError("MultiObject-2b public digest mismatch")
    ...
    if int(m.get("added_fixations", -1)) != 1 or int(m.get("growth_iterations_added", -1)) != 0:
        raise AssertionError("MultiObject-2b is not the declared one-look seed experiment")
    if not m.get("new_object_instantiated") or not m.get("existing_objects_read_only"):
        raise AssertionError("MultiObject-2b did not preserve the existing scene while seeding")
    target_id = int(m.get("selected_object_id", -1))
    if target_id <= 0 or target_id in set(public.PREEXISTING_OBJECT_IDS):
        raise AssertionError("invalid parent-selected target object id")
    ids_after = [int(x) for x in m.get("object_ids_after", [])]
    if set(ids_after) != set(public.PREEXISTING_OBJECT_IDS + (target_id,)):
        raise AssertionError("MultiObject-2b object set changed")
    sg = json.loads((parent / "scene_graph.json").read_text())
    objs = {int(o["object_id"]): o for o in sg.get("objects", [])}
    if set(objs) != set(ids_after):
        raise AssertionError("MultiObject-2b scene graph/object ids disagree")
    for oid in public.PREEXISTING_OBJECT_IDS:
...
def _run_scene_blender(args, step: int, gaze: tuple[float, float], profile: str) -> Path:
    ...
    if p.returncode != 0:
        raise RuntimeError(f"MultiObject-2c Blender fixation {step} failed; see render log")
    ...
    if not rr.get("complete") or int(rr.get("seed", -1)) != public.SEED or int(rr.get("step", -1)) != step:
        raise RuntimeError("incomplete or wrong MultiObject-2c Blender record")
    if abs(float(rr["yaw_deg"]) - yaw) > 1e-8 or abs(float(rr["pitch_deg"]) - pitch) > 1e-8:
        raise RuntimeError("Blender record gaze mismatch")
    return case


def _load_existing_maps(parent: Path, objs: dict[int, dict], target_id: int):
    maps = {}
    paths = {}
    hashes = {}
    for oid in public.PREEXISTING_OBJECT_IDS:
...
def _write_scene_graph(path: Path, parent: Path, target_id: int,
    ...
    json_write(path, {
        "schema": "MultiObject2c-scene-graph-v1",
        "fixed_head": True,
        "static_scene": True,
        "objects": objects,
...
def execute(args) -> dict:
    ...
    if public.FUSION != dict(frozen_public.FUSION):
        raise AssertionError("MultiObject-2c changed the frozen 12 mm FSG6f fusion rule")

    pm, _parent_scene, target_id, objs = _validate_parent(args.parent)
    ...
    existing_maps, existing_paths, existing_hash_before, seed_patch_path = _load_existing_maps(
        args.parent, objs, target_id
    )
    ...
    if len(p0.instance_id) and set(np.unique(p0.instance_id).tolist()) != {target_id}:
        raise AssertionError("MultiObject-2b seed acquisition is not pure for selected object")
    with np.load(seed_patch_path, allow_pickle=False) as z:
        if not np.array_equal(np.asarray(z["xyz_h"], np.float32), np.asarray(p0.xyz_h, np.float32)):
            raise AssertionError("saved MultiObject-2b seed xyz no longer matches its acquisition")
        if not np.array_equal(np.asarray(z["instance_id"]), np.asarray(p0.instance_id)):
            raise AssertionError("saved MultiObject-2b seed ids no longer match its acquisition")
    ...
    policy_trace = []
    seed_tex = _texture_stats(rec, target_id)
    patch_stats = [{
        ...
        "source": "MultiObject-2b prescribed scene-selected seed",
        **seed_tex,
    }]
    association_stats = [{
        "global_step": int(seed_step),
    ...
    json_write(args.out / f"object_{target_id}_policy_trace.json", {
        "policy": "frozen_fsg6f_via_parent_selected_target_label_adapter",
        "target_object_id": int(target_id),
        "trace": policy_trace,
    })

    chart_objects = [(oid, existing_maps[oid].xyz_h) for oid in public.PREEXISTING_OBJECT_IDS]
    ...
    _write_scene_graph(
        args.out / "scene_graph.json", args.parent, target_id,
    ...
    if parent_hash_after != parent_hash_before:
        raise AssertionError("MultiObject-2b parent changed during growth")
    for oid in public.PREEXISTING_OBJECT_IDS:
    ...
    scientific_stop = bool(termination != "object3_watchdog" and policy_trace[-1].get("stop"))
    recovery_values = [
    ...
    manifest = {
        ...
        "static_scene": True,
        "truth_opened": False,
        "preexisting_object_ids": list(public.PREEXISTING_OBJECT_IDS),
        "selected_object_id": int(target_id),
        "object_ids": list(public.PREEXISTING_OBJECT_IDS + (target_id,)),
        ...
        "watchdog_selected_object_fixations": public.OBJECT3_WATCHDOG_FIXATIONS,
        "policy_adapter": "parent_selected_id_to_frozen_fsg6f_target_label",
        "policy_source_modified": False,
        "growth_history_starts_at_multiobject2b_seed": True,
...
def main() -> None:
    ...
    print("[multiobject2c-run] MULTIOBJECT2C_COMPLETE " + json.dumps({
