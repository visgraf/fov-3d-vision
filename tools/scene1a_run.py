"""Run one Stage II / Scene-1a seeded multi-object active reconstruction trial."""
...
def save_scene_ply(path:Path,maps:dict[int,SurfaceMap])->None:
    ...
    with path.open("w",encoding="ascii") as f:
        f.write("ply\nformat ascii 1.0\ncomment fixed head frame H; Stage II Scene-1a\n")
...
def write_scene_overview(path:Path,maps:dict[int,SurfaceMap],gazes:list[tuple[float,float]],targets:list[int])->None:
    ...
    cv2.putText(canvas,"Scene-1a final persistent maps and physical fixations",(55,30),cv2.FONT_HERSHEY_SIMPLEX,.60,(20,20,20),1,cv2.LINE_AA)
    cv2.imwrite(str(path),canvas)

def run_blender(args,step:int,target:int,gaze:tuple[float,float],root:Path):
    yaw,pitch=gaze; out=root/f"fix_{step:02d}"
    cmd=[args.blender,"-b","--python-exit-code","1","-P","tools/scene1a_render_fix.py","--","--out",str(out),"--profile",args.profile,"--fixture",args.fixture,"--seed",str(args.seed),"--step",str(step),"--target-object",str(target),"--yaw",f"{yaw:.12g}","--pitch",f"{pitch:.12g}","--device",args.device]
    ...
    if not rr.get("complete") or rr.get("fixture")!=args.fixture or int(rr.get("seed"))!=args.seed or int(rr.get("target_object_id"))!=target or abs(float(rr["yaw_deg"])-yaw)>1e-8 or abs(float(rr["pitch_deg"])-pitch)>1e-8:
        raise RuntimeError("incomplete or wrong Scene-1a Blender record")
    return case,rr


def _state(oid:int,maps,hist,last_obs,last_gaze)->dict:
    return {"object_id":oid,"map":maps[oid],"observation_history":hist[oid],"last_target_observation":last_obs[oid],"last_target_gaze_deg":last_gaze[oid]}
...
def execute(args)->dict:
    ...
    if public.FUSION!=frozen_public.FUSION: raise AssertionError("Scene-1a must preserve frozen FSG6f/FSG3 fusion")
    if public.FROZEN_OBJECT_POLICY_ID!=frozen_public.SPEC_ID: raise AssertionError("Scene-1a must use frozen FSG6f object controller")
    if args.fixture not in public.FIXTURES or args.seed not in public.SEEDS: raise ValueError("fixture/seed outside Scene-1a schedule")
    ...
    target_counts={oid:0 for oid in public.OBJECT_IDS}; global_gazes=[]; target_sequence=[]; patch_stats=[]; assoc_stats=[]; scheduler_trace=[]
    ...
    for step in range(public.MAX_SCENE_FIXATIONS):
        if step<len(seeds): target,gaze=seeds[step]; phase="prescribed_seed"
            ...
            if next_active is None:
                states={oid:_state(oid,maps,histories,last_obs,last_gaze) for oid in public.OBJECT_IDS}
        ...
        target_counts[int(target)]+=1; last_obs[int(target)]=robs; last_gaze[int(target)]=tuple(gaze)
    ...
    json_write(args.out/"scene_policy_trace.json",{"policy":"scene scheduler over frozen FSG6f object proposals","trace":scheduler_trace})
    final_states={}
    states={oid:_state(oid,maps,histories,last_obs,last_gaze) for oid in public.OBJECT_IDS}
    ...
    manifest={"schema":"Scene1a-prediction-v1","instrument":public.INSTRUMENT_ID,"public_spec_sha256":public.public_digest(),"profile":args.profile,"fixture":args.fixture,"seed":args.seed,"fixed_head":True,"static_scene":True,
              "fixation_gazes_deg":[list(g) for g in global_gazes],"target_object_sequence":target_sequence,"prescribed_seed_fixations":len(seeds),"termination_reason":termination,"target_counts":{str(k):int(v) for k,v in target_counts.items()},"final_object_policy_states":final_states,
              "patch_stats":patch_stats,"association_stats":assoc_stats,"truth_opened":False,"policy_inputs":["three persistent object maps","current/past binocular oracle instance masks and raw support","calibration","global physical fixation history","frozen FSG6f per-object proposals"],
    ...
    print("[scene1a-run] COMPLETE",json.dumps({"fixture":args.fixture,"seed":args.seed,"fixations":len(global_gazes),"targets":target_sequence,"termination":termination,"samples":samples},sort_keys=True),flush=True)
    return manifest
