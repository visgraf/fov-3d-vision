"""Structural comparator for completed MultiObject-2d read-only audits."""
...
def main() -> None:
    ...
    status = "MULTIOBJECT2D_COMPLETE" if not fails else "MULTIOBJECT2D_INTEGRITY_FAIL"
    print("[multiobject2d-compare]", status, json.dumps({"records": rows, "structural_fails": fails}, sort_keys=True))
