"""Truth-free public contract for FSG6 3D surface-frontier growth.

FSG6 keeps the closed FSG1 RGB-D instrument and FSG3 surfel fusion fixed, but
replaces the horizontal image-edge controller with a truth-free frontier derived
from the persistent 3D surfel map.  Candidate gazes live on a local yaw/pitch
lattice and oracle segmentation is used only to decide whether the object still
continues through the corresponding current-image edge(s).
"""
from __future__ import annotations
import hashlib
import json

SPEC_ID = "FSG6-3d-surface-frontier-v1"
INSTRUMENT_ID = "FSG1-HDR-SGBM-one-original-update-original-validity-v1"
OBJECT_ID = 91
BACKGROUND_ID = 92
FIXTURES = ("diag_up_right", "diag_down_left")
SEEDS = (701, 743)
DEFAULT_SPP = {"small": 64, "full": 256}
SEED_GAZE_DEG = {
    "diag_up_right": (-8.0, -8.0),
    "diag_down_left": (+8.0, +8.0),
}
...
SURFACE_FRONTIER = {
    # Candidate gaze lattice.  A move may change yaw, pitch, or both by one
    # component step; thus diagonal moves have sqrt(2)*5 deg angular magnitude.
    "component_step_deg": 5.0,
    "yaw_min_deg": -25.0,
    "yaw_max_deg": +25.0,
    "pitch_min_deg": -20.0,
    "pitch_max_deg": +20.0,
    # Oracle mask is only an object-continuation check, not the frontier score.
    "edge_band_fraction": 0.04,
    "edge_object_fraction_min": 0.15,
    # Truth-free 3D frontier extraction from the persistent surfel map.
    "voxel_m": 0.025,
    ...
    # Candidate ranking: among 3D-frontier-supported and segmentation-permitted
    # moves, maximize angular footprint outside the map's robust yaw/pitch box.
...
TARGETS = {
    ...
    # A horizontal-only controller cannot satisfy this on the new diagonal
    # fixture family.  This is the experiment's explicit 3D-gaze requirement.
...
def fixture_index(fixture: str) -> int:
    if fixture not in FIXTURES:
        raise ValueError(f"unknown FSG6 fixture {fixture}")
...
def render_seed(fixture: str, seed: int, yaw_deg: float, pitch_deg: float, eye_id: int) -> int:
    ...
    if seed not in SEEDS:
        raise ValueError("seed not in frozen FSG6 schedule")
    if eye_id not in (0, 1):
        raise ValueError("eye_id must be 0 or 1")
    p = SURFACE_FRONTIER
    y = float(yaw_deg); q = float(pitch_deg)
    if not (p["yaw_min_deg"] - 1e-9 <= y <= p["yaw_max_deg"] + 1e-9):
        raise ValueError("yaw outside frozen FSG6 range")
    if not (p["pitch_min_deg"] - 1e-9 <= q <= p["pitch_max_deg"] + 1e-9):
        raise ValueError("pitch outside frozen FSG6 range")
    ...
    return 1_000_000 * int(seed) + 600_000 * fixture_index(fixture) + 1000 * iy + 2 * ip + eye_id
