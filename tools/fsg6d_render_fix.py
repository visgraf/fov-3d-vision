"""Blender-side acquisition of one FSG6d yaw/pitch fixation pair."""
...
import fsg6d_scene as scene_spec
...
def acquire_one(args: argparse.Namespace) -> dict:
    if args.fixture not in public.FIXTURES or args.seed not in public.SEEDS:
        raise ValueError("fixture/seed outside frozen FSG6d schedule")
    if args.step < 0 or args.step >= public.MAX_BUDGET_FIXATIONS:
        raise ValueError("step outside frozen FSG6d budget")
    ...
    acq={"schema":"FSG6d-fixation-acquisition-v1","source":backend.source,"case":case,"step":args.step,"fixture":args.fixture,"profile":args.profile,
    ...
    run={"schema":"FSG6d-fixation-run-v1","source":backend.source,"complete":True,"created_utc":dt.datetime.now(dt.timezone.utc).isoformat(),
    ...
    print(f"[fsg6d-render] COMPLETE fixture={args.fixture} seed={args.seed} step={args.step} yaw={args.yaw:.3f} pitch={args.pitch:.3f} samples={acq['primary_camera_samples']}",flush=True)
    return run
...
if __name__=="__main__":
    try: main()
        ...
        traceback.print_exc(); print("[fsg6d-render] FAILED",flush=True); sys.stdout.flush(); sys.stderr.flush(); os._exit(1)
