"""Continue exact Reality Check 1 records, treating an empty target look as evidence.
...
post-six views are newly acquired.  Unlike Reality Check 2, a fixation with fewer
than the inherited 100 reconstructed target points is not an exception: the
completed binocular observation is recorded, no target points are fused, the map
is left unchanged, and frozen FSG6f chooses again using that negative evidence.
"""
...
def _validate_parent(parent: Path) -> tuple[dict, list[dict], list[tuple[float, float]]]:
    ...
    if m.get("instrument") != public.INSTRUMENT_ID or m.get("frozen_object_policy") != public.FROZEN_POLICY_ID:
        raise AssertionError("parent instrument/policy differs from Reality Check 2b")
    if m.get("termination_reason") != "max_fixations":
        raise AssertionError("Reality Check 2b requires a parent interrupted only by the six-look limit")
...
def run_blender(args, step: int, gaze: tuple[float, float], acq_root: Path):
    ...
    # Reuse the exact Reality Check 2 renderer: scene, texture, instrument and RNG
    # mapping are unchanged.  Reality Check 2b changes only post-observation semantics.
    cmd = [
        args.blender, "-b", "--python-exit-code", "1", "-P", "tools/reality2_render_fix.py", "--",
...
def execute(args) -> dict:
    ...
    if public.FUSION != parent_public.FUSION:
        raise AssertionError("Reality Check 2b changed fusion")

    pm, parent_trace, gazes = _validate_parent(args.parent)
    args.seed = int(pm["seed"])
    args.profile = str(pm["profile"])
    if args.seed not in public.SEEDS:
        raise AssertionError("parent seed outside Reality Check 2b schedule")

    # Carry exact saved state forward.  No Reality Check 1 view is rerendered.
    ...
    for step in range(public.PARENT_FIXATIONS):
        src = args.parent / "maps" / f"map_{step:02d}.npz"
        dst = args.out / "maps" / f"map_{step:02d}.npz"
        shutil.copy2(src, dst)
        mm = load_map(src)
        snapshots.append(mm.xyz_h.copy())
        supports.append(mm.support_count.copy())
    ...
    while len(gazes) < public.WATCHDOG_TOTAL_FIXATIONS:
        ...
        new_samples += int(rr["primary_camera_samples"])
        new_render_seconds += float(rr["total_wall_seconds"])
        c, obs = hdr.read_observation(case)
        rec, meta, state = compute_once(c, obs)
        pid = f"fix_{step:02d}"
        p = fsg6run.patch_from_record(pid, rec)
        ...
        contract = public.empty_observation_contract(len(p.xyz_h))
        empty_target = bool(contract["empty_target_observation"])
        before = sm

        if contract["fuse_target_points"]:
            sm, assoc = fuse(
                sm, p, public.OBJECT_ID,
            ...
            assoc_stats.append({
                ...
                "patch_id": pid,
                "fused": True,
                "empty_target_observation": False,
                "input_points": int(assoc["input_points"]),
                "matched": int(assoc["matched"]),
                "new": int(assoc["new"]),
            ...
            # Negative perception is still an observation.  It must not add or
            # mutate target surfels; its binocular masks/support enter history below.
            empty_steps.append(step)
            if not _map_arrays_equal(before, sm):
                raise AssertionError("empty target observation changed the persistent map before recording")
            assoc_stats.append({
                "step": step,
                "patch_id": pid,
                "fused": False,
                "empty_target_observation": True,
                "input_points": int(len(p.xyz_h)),
                "matched": 0,
                "new": 0,
                "affected_surfels": 0,
                "new_fraction": 0.0,
                "overlap_median_distance_m": None,
                "overlap_p95_distance_m": None,
                "idempotent_replay": None,
                "reason": "negative binocular observation retained; no target fusion",
            })
        ...
        patch_stats.append({
            ...
            "yaw_deg": gaze[0],
            "pitch_deg": gaze[1],
            "object_reference_count": nref,
            "object_valid_count": nvalid,
            "object_measurement_fraction": cov,
            "point_count": len(p.xyz_h),
            "empty_target_observation": empty_target,
            "fused": bool(contract["fuse_target_points"]),
            "instrument": public.INSTRUMENT_ID,
        })

        # This is the scientific change: even an empty target look becomes a
        # completed binocular observation in persistent history and a visited gaze.
        ...
        decision = policy.choose_next(
            gaze[0], gaze[1], c,
            rec["instance_id"], rec["raw_support_L"], ids_R, raw_support_R,
            sm.xyz_h, gazes, observation_history,
        )
        decision["step"] = step
        decision["observation_was_empty_target"] = empty_target
        decision["empty_observation_semantics"] = contract["action"]
    ...
    json_write(args.out / "policy_trace.json", {
        "policy": "frozen_fsg6f_reality1_exact_continuation_empty_look_is_evidence",
        "trace": policy_trace,
    })
    parent_hashes = {
        name: _sha256(args.parent / name)
        for name in ("prediction_manifest.json", "policy_trace.json", "surface_map.npz")
    }
    manifest = {
        "schema": "RealityCheck2b-prediction-v1",
        "instrument": public.INSTRUMENT_ID,
        "frozen_object_policy": public.FROZEN_POLICY_ID,
        ...
        "continued_from_parent_without_rerender": True,
        "parent_fixation_count": public.PARENT_FIXATIONS,
        "watchdog_total_fixations": public.WATCHDOG_TOTAL_FIXATIONS,
        "scientific_stopping_rule": "frozen FSG6f no_frontier",
        "empty_target_point_limit": public.EMPTY_TARGET_POINT_LIMIT,
        "empty_observation_semantics": public.PUBLIC_SPEC["empty_observation_semantics"],
        ...
        "static_scene": True,
        "policy_inputs": [
            "persistent map xyz_h",
            "current rectified oracle target mask L/R",
            "raw calibration support L/R",
            "calibration",
            "fixation history including empty looks",
            "completed binocular oracle/support history including empty looks",
        ],
    ...
    print("[reality2b-run] COMPLETE", json.dumps({
        "seed": args.seed,
        "total_fixations": len(gazes),
        "new_fixations": len(gazes) - public.PARENT_FIXATIONS,
        "empty_observation_steps": empty_steps,
        "gazes": gazes,
