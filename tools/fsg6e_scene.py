"""Evaluator-only rolled cylindrical ribbons for FSG6e.

FSG6e uses fresh non-mirror curved fixtures and an evaluator-side analytic
...
PREFLIGHT_TRACE = {
    "closure_up_right": [(-8.0,-7.0), (-3.0,-2.0), (2.0,3.0), (7.0,8.0)],
    "closure_down_left": [(8.0,7.0), (3.0,2.0), (-2.0,-3.0), (-7.0,-8.0)],
}
...
def _strip_quad(fixture: str, j: int) -> dict:
    ...
    return {
        "name": f"fsg6e_{fixture}_strip_{j:02d}",
        "vertices_h": verts,
...
def scene_objects(fixture: str) -> list[dict]:
    ...
    out.append(quad([0.0, 0.0, BACKGROUND_Z_M], [1, 0, 0], [0, 1, 0],
                    ...
                    public.BACKGROUND_ID, f"fsg6e_{fixture}_background"))
    return out
...
def validate_mesh(fixture: str, mesh: dict) -> None:
    ...
    if int((ids == public.OBJECT_ID).sum()) != 2 * STRIP_COUNT or int((ids == public.BACKGROUND_ID).sum()) != 2:
        raise ValueError("unexpected FSG6e exported triangle counts")
    if set(ids.tolist()) != {public.OBJECT_ID, public.BACKGROUND_ID}:
        raise ValueError("unexpected FSG6e object IDs")
    ...
    if exp_obj.shape != got_obj.shape or exp_bg.shape != got_bg.shape:
        raise ValueError("exported FSG6e mesh vertex population disagrees")
    if np.max(np.linalg.norm(exp_obj - got_obj, axis=1)) > 2e-5 or np.max(np.linalg.norm(exp_bg - got_bg, axis=1)) > 2e-5:
        raise ValueError("exported Blender mesh disagrees with FSG6e specification")
...
def _direction_probe_one_eye(ids: np.ndarray, sup: np.ndarray, dx: int, dy: int) -> dict:
    ...
    possible under the exact FSG6e corridor semantics.
    """
    import fsg6e_frontier as policy
...
def persistent_policy_preflight(fixture: str) -> dict:
    """Evaluator-only closed-loop sanity check using actual FSG6e policy semantics.
    ...
    if last_bundle is None: raise AssertionError("empty FSG6e policy preflight")
    final_gaze,c,L,R,final_map,visited,final_decision=last_bundle
    # Boundary history must be load-bearing: removing it at the same final state
    # must leave at least one candidate, reproducing the FSG6d termination gap.
    no_history=policy.choose_next(final_gaze[0],final_gaze[1],c,L[0],L[1],R[0],R[1],final_map,visited,[])
    return {"gazes":gazes,"records":records,"stop":bool(final_decision["stop"]),
            ...
            "without_history_stops":bool(no_history["stop"]),
            "without_history_candidate_count":int(len(no_history["candidates"]))}

def self_test() -> None:
    ur=angular_bounds("closure_up_right"); dl=angular_bounds("closure_down_left")
    for name,b in (("closure_up_right",ur),("closure_down_left",dl)):
    ...
    if np.allclose(np.array(dl),np.array([-ur[1],-ur[0],-ur[3],-ur[2]]),atol=1e-3):
        raise AssertionError("FSG6e fixtures unexpectedly collapsed to an exact mirror pair")
    ...
    for f in public.FIXTURES:
        ...
        if not pf["stop"] or pf["reason"]!="no_frontier":
            raise AssertionError(f"persistent FSG6e policy preflight did not terminate: {f} {pf}")
        if not (public.TARGETS["active_fixation_count_min"] <= len(pf["gazes"]) <= public.TARGETS["active_fixation_count_max"]):
            raise AssertionError(f"persistent FSG6e policy preflight fixation count invalid: {f} {pf}")
        if pf["coverage"] < public.TARGETS["final_truth_coverage_min"]:
            raise AssertionError(f"persistent FSG6e policy preflight coverage too low: {f} {pf}")
        if pf["pitch_span_deg"] < public.TARGETS["pitch_span_min_deg"]:
            raise AssertionError(f"persistent FSG6e policy preflight stayed too horizontal: {f} {pf}")
        if pf["without_history_stops"] or pf["without_history_candidate_count"] < 1:
            raise AssertionError(f"FSG6e historical boundary state is not load-bearing at final preflight state: {f} {pf}")

    horiz_r=[(-8+5*k,-7) for k in range(5)]; horiz_l=[(8-5*k,7) for k in range(5)]
    hc=max(ideal_angular_coverage("closure_up_right",horiz_r),ideal_angular_coverage("closure_down_left",horiz_l))
    if hc>=0.65:
        raise AssertionError("FSG6e fixture no longer requires two-dimensional gaze")
    chord=max(max_strip_chord_error_m(f) for f in public.FIXTURES)
    if chord>0.0002: raise AssertionError(f"FSG6e render tessellation too coarse: {chord} m")
    for f in public.FIXTURES:
        if float(np.max(surface_distance(f,truth_points(f))))>1e-10:
            raise AssertionError("FSG6e analytic truth points reject themselves")
    print(f"[fsg6e-scene] PASS up_right=[{ur[0]:.3f},{ur[1]:.3f}]x[{ur[2]:.3f},{ur[3]:.3f}] "
          f"down_left=[{dl[0]:.3f},{dl[1]:.3f}]x[{dl[2]:.3f},{dl[3]:.3f}] horizontal_ideal_max={hc:.3f} "
          f"chord_max_mm={1000*chord:.3f} corridor_preflight=true persistent_state_preflight=true raw_frontier_termination_rejected=true "
