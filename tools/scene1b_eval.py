"""Post-hoc evaluation for one Stage II / Scene-1b multi-object run."""
...
def evaluate(root:Path,out:Path,mode:str)->dict:
    ...
    if fixture not in public.FIXTURES or seed not in public.SEEDS or manifest.get("instrument")!=public.INSTRUMENT_ID: raise ValueError("wrong Scene-1b fixture/seed/instrument")
    if manifest.get("fixed_head") is not True or manifest.get("static_scene") is not True: raise ValueError("Scene-1b base assumptions violated")
    ...
    # This is the load-bearing Scene-1b check: an object with higher utility is
