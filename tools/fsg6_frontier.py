"""Truth-free 3D surfel-frontier controller for FSG6.
...
def angular_coordinates(xyz_h: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    p = p[good]
    if len(p) < 100:
        raise ValueError("too few finite forward map points for FSG6 policy")
...
def choose_next(current_yaw_deg: float, current_pitch_deg: float, calibration: dict,
                instance_id: np.ndarray, raw_support: np.ndarray, map_xyz_h: np.ndarray,
...
def self_test() -> None:
    ...
    # Same all-edge object mask, different persistent map states: map state must change direction.
    ids=np.full((n,n), public.OBJECT_ID, np.int32)
    a=choose_next(0.0,0.0,cal,ids,sup,_xyz_patch((-11,1),(-11,1)),[(0.0,0.0)])
    b=choose_next(0.0,0.0,cal,ids,sup,_xyz_patch((-1,11),(-1,11)),[(0.0,0.0)])
    ...
    # Resolved mask: no object in any image edge -> no candidate even though map has a geometric boundary.
    ids2=np.full((n,n), public.OBJECT_ID, np.int32); k=10
    ids2[:k,:]=public.BACKGROUND_ID; ids2[-k:,:]=public.BACKGROUND_ID; ids2[:,:k]=public.BACKGROUND_ID; ids2[:,-k:]=public.BACKGROUND_ID
    z=choose_next(0.0,0.0,cal,ids2,sup,_xyz_patch((-5,5),(-5,5)),[(0.0,0.0)])
    if not z["stop"] or z["reason"] != "no_frontier":
        raise AssertionError("resolved object boundary must stop 3D frontier exploration")
    print("[fsg6-frontier] PASS map_state_changes_2d_direction=true resolved_boundary_stops=true")
