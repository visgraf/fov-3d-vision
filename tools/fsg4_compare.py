"""Aggregate the four prospectively fixed FSG4 paired full trials."""
...
def write_visual(path:Path, rows:list[dict])->None:
    ...
    for j,r in enumerate(rows):
        ...
        def pts(c):
            return [(55+i*(630//4),int(y0+160-float(v)*118)) for i,v in enumerate(c)]
...
def compare(pair_roots:list[Path],out:Path)->dict:
    ...
    rows=[];seen=set()
    for p in pair_roots:
        r=json.loads((p.resolve()/"pair.json").read_text())
        key=(r['fixture'],int(r['seed']))
        if key in seen:raise ValueError("duplicate FSG4 pair")
        seen.add(key);rows.append(r)
    if seen!=EXPECTED:raise ValueError(f"FSG4 full pair set incomplete: got {sorted(seen)} expected {sorted(EXPECTED)}")
    ...
    gains=np.array([r['auc_gain'] for r in rows],float)
    finals=np.array([r['final_coverage_gain'] for r in rows],float)
    wins=int((gains>0).sum())
    mean_auc=float(gains.mean());mean_final=float(finals.mean())
    ...
    result={
        'schema':'FSG4-comparison-v1','pairs':rows,'pair_count':len(rows),'pair_auc_wins':wins,
        'mean_auc_gain':mean_auc,'mean_final_coverage_gain':mean_final,
        'auc_gain_by_pair':{f"{r['fixture']}-seed{r['seed']}":r['auc_gain'] for r in rows},
        'final_coverage_gain_by_pair':{f"{r['fixture']}-seed{r['seed']}":r['final_coverage_gain'] for r in rows},
        'targets':t,'fails':fails,'status':'FSG4_INCREMENT4_PASS' if not fails else 'FSG4_INCREMENT4_FAIL',
        'claim_scope':'paired efficiency on two mirrored planar placements x two Monte-Carlo seeds; not policy optimality or a population estimate',
    }
    out.mkdir(parents=True);json_write(out/'comparison.json',result);write_visual(out/'coverage_vs_budget.png',rows)
    print('[fsg4-compare] '+result['status'],json.dumps(result,sort_keys=True),flush=True)
    return result
