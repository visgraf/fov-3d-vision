"""Threshold-free descriptive outcome helpers for MultiObject-3g."""
...
def measurement_status(empty: bool, new_surfels: int, matched_surfels: int) -> str:
    ...
    if int(new_surfels) > 0:
        return "LOCAL_ACTION_ADDED_GEOMETRY"
    if int(matched_surfels) > 0:
        return "LOCAL_ACTION_REMEASURED_EXISTING_GEOMETRY"
    return "LOCAL_ACTION_MEASURED_NO_ASSOCIATION"
...
def experiment_stop() -> str:
    return "BOUNDED_AFTER_ONE_EXECUTED_RETURNED_ACTION_AND_ONE_NEXT_DECISION"


def self_test() -> None:
    assert measurement_status(False, 3, 4) == "LOCAL_ACTION_ADDED_GEOMETRY"
    assert measurement_status(False, 0, 4) == "LOCAL_ACTION_REMEASURED_EXISTING_GEOMETRY"
    ...
    print("[multiobject3g-progress] PASS threshold_free=true one_action=true one_next_decision=true auto_loop=false")
