"""Acquire the frozen FSG1g fresh validation suite in Blender.
...
def acquire_final(args: argparse.Namespace) -> dict:
    if args.seed not in spec.SEEDS or (args.profile == "small" and args.seed != spec.SEEDS[0]):
        raise ValueError("seed/profile combination not in FSG1g prospective schedule")
    ...
    try:
        ...
        raise
    print("[fsg-final-render] COMPLETE spec=" + spec.spec_digest(), flush=True)
    return run
...
if __name__ == "__main__":
    try: main()
        ...
        traceback.print_exc(); print("[fsg-final-render] FAILED", flush=True)
