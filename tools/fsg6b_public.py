"""Truth-free public contract for FSG6b binocular 3D surface-frontier validation.

FSG6a demonstrated that the persistent 3D surfel frontier can drive genuinely
2D gaze, but its auxiliary object-continuation veto used only the left eye and
therefore was not invariant to binocular eye swapping. FSG6b keeps the frontier,
stereo instrument, fusion rule, thresholds and gaze lattice fixed; it changes
only the continuation veto to use symmetric evidence from both rectified eyes.
"""
from __future__ import annotations
import hashlib
import json

SPEC_ID = "FSG6b-binocular-surface-frontier-v1"
INSTRUMENT_ID = "FSG1-HDR-SGBM-one-original-update-original-validity-v1"
OBJECT_ID = 101
BACKGROUND_ID = 102
FIXTURES = ("fresh_up_right", "fresh_down_left")
SEEDS = (809, 853)
DEFAULT_SPP = {"small": 64, "full": 256}
SEED_GAZE_DEG = {
    "fresh_up_right": (-8.0, -7.0),
    "fresh_down_left": (+8.0, +7.0),
}
...
# Numerically identical to FSG6a. Do not tune these constants in FSG6b.
SURFACE_FRONTIER = {
...
PUBLIC_SPEC = {
    ...
    "binocular_continuation_veto": "max per-edge object fraction across rectified L/R eyes",
    "continuation_threshold_unchanged_from_fsg6a": True,
    "no_icp": True,
    "no_hole_fill": True,
...
def fixture_index(fixture: str) -> int:
    if fixture not in FIXTURES:
        raise ValueError(f"unknown FSG6b fixture {fixture}")
...
def render_seed(fixture: str, seed: int, yaw_deg: float, pitch_deg: float, eye_id: int) -> int:
    ...
    if seed not in SEEDS:
        raise ValueError("seed not in frozen FSG6b schedule")
    ...
    if not (p["yaw_min_deg"] - 1e-9 <= y <= p["yaw_max_deg"] + 1e-9):
        raise ValueError("yaw outside frozen FSG6b range")
    if not (p["pitch_min_deg"] - 1e-9 <= q <= p["pitch_max_deg"] + 1e-9):
        raise ValueError("pitch outside frozen FSG6b range")
