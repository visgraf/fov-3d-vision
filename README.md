...
| Step | State |
| --- | --- |
| FSG1: single-patch local stereo | Run 2026-09-19 (Blender 5.2.1, OPTIX) across D-FSG1a..g. **Increment 1 is NOT closed; nothing adopted, no fusion.** Seed-17 baselines FAIL; FSG1b traced the coverage miss to over-exposure and FSG1c's fixed HDR encoding cleared it on those records. FSG1d tested that candidate on new fixtures at seeds 31/73: `FROZEN_CANDIDATE_VALIDATION_FAIL` (3.2 m background missed accuracy; the half-occlusion check, first exercised there, leaked 2 core pixels at seed 31). FSG1e explained both: the error tail IS the +/-0.75 px refinement cap on a near-integer-phase surface, while refinement RESCUES the 3.4 m background, and all six leaks passed an interpolated LR check their endpoints failed. FSG1f tested one update plus footprint-supported reciprocity on the seven diagnostic pairs: `CANDIDATE_PASS_ON_DIAGNOSTIC_RECORDS` (backgrounds 1.053%/3.123% -> 0.383%/1.606%, zero occlusion leaks) - development-set evidence only. FSG1g, the final prospective validation of the simpler one-update instrument on fresh fixtures/seeds, **STOPPED at the smoke stage on a fixture-integrity failure**: `half-occlusion NOT_EXERCISED: raw`. Both new occluders place their nearest foreground edge 10.2-10.7 deg off axis while the accepted core spans only +/-6.0 deg at either profile, so no occluding edge enters the measurement (0 raw / 0 core singly-visible vs a required 256/128 at full). Profile-independent, so the two full acquisitions (2,516,582,400 samples) were not run. The four phase planes are correct (22.00/22.25/22.50/22.75 px) and passed their smoke gates at 100% coverage. Increment 2 is NOT authorized. Next step is a fixture correction decided by Luiz/Chat. See `docs/fsg1-single-patch.md`, `docs/fsg1-coverage-audit.md`, `docs/fsg1-hdr-candidate.md`, `docs/fsg1-prospective-validation.md`, `docs/fsg1-stage-visibility-audit.md`, `docs/fsg1-supported-candidate.md`, `docs/fsg1-final-validation.md`. |
