...
| Step | State |
| --- | --- |
| FSG1: single-patch local stereo | Run 2026-09-19 (Blender 5.2.1, OPTIX) across D-FSG1a..f. **Nothing is adopted, FSG1 is not closed, no fusion.** Seed-17 baselines FAIL; FSG1b traced the coverage miss to over-exposure and FSG1c's fixed HDR encoding cleared it on those records. FSG1d tested that frozen candidate on two NEW fixtures at seeds 31/73: `FROZEN_CANDIDATE_VALIDATION_FAIL` - the 3.2 m background missed accuracy on both seeds and the half-occlusion check (first exercised there, 4,608 raw / 3,528 core) leaked 2 core pixels at seed 31. FSG1e, a read-only replay, explained both: the tail IS the +/-0.75 px refinement cap on a near-integer-phase surface (~88% of >3% errors; raw SGBM was 0.099%/0.359%), while refinement RESCUES the 3.4 m background, and all six leaks passed an interpolated LR check their endpoints failed. FSG1f then tested one update plus footprint-supported reciprocity on the same seven pairs, zero new samples: `CANDIDATE_PASS_ON_DIAGNOSTIC_RECORDS` - every gate met on all seven, backgrounds 1.053%/3.123% -> 0.383%/1.606%, the 3.4 m regression case improved to 0.333%/1.176%, and 0 of 4,608 raw / 3,528 core occlusion pixels accepted. Costs charged: the footprint rule removes ~2.5% of interior coverage (94.6%, still >90%) and ~half the boundary population; attribution between the one-step validity and the new vetoes is not separable on these records. This is development/diagnostic data, not validation - seeds 17/31/73 are all inspected. Next step is a decision by Luiz/Chat. See `docs/fsg1-single-patch.md`, `docs/fsg1-coverage-audit.md`, `docs/fsg1-hdr-candidate.md`, `docs/fsg1-prospective-validation.md`, `docs/fsg1-stage-visibility-audit.md`, `docs/fsg1-supported-candidate.md`. |
