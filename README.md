...
## Foveal Surface Growing - FSG1..FSG4 (opt-in experiment)
...
| FSG4: active vs fixed scan (Increment 4) | **OPEN - `FSG4_INCREMENT4_FAIL` 2026-09-19; Increment 4 is NOT closed and no next experiment is authorized.** FSG4a stopped at the paired-noise gate (repeated OptiX renders are not bit-reproducible: exact seeds and masks, RGB differing by 1-2 float32 ulp). FSG4b repaired it **without relaxing the gate** - the active policy runs first with no cache and the fixed scan reuses the exact saved active acquisition at shared yaws - and `np.array_equal` is unchanged while the gate additionally requires a declared reuse flag; the repaired `--negative pairing` control still fails on one mutated float. All four full pairs then ran. **The comparison gates pass emphatically**: active AUC wins **4/4**, mean AUC advantage **0.1649**, mean final-coverage advantage **0.1655**, active logical samples <= scan everywhere, 3 shared yaws per pair all exact and reused. Metric geometry passes for both policies (plane medians 3.97-4.35 mm active, 3.81-4.20 mm scan; every map pure ID 81; every fusion idempotent). **But the inherited FSG3 active-run contract fails on `case_b` at both seeds, on the terminal patch only** - new fraction 0.039 vs >=0.05 and terminal gain 1.28 pp vs >=2 pp - because the policy is already at 98.7% after four looks and its fifth catches only the last sliver. Note the tension: that same trajectory gives the largest advantage in the experiment (AUC gain 0.230, 100% coverage). Nothing tuned; all four pairs preserved. See `docs/fsg4-increment4.md` and `docs/fsg4b-pairing-reuse.md`. |
...
Increment 2 added two-patch accumulation into that fixed map and Increment 3 an
active frontier policy that selects and stops its own fixations. Increment 4, the
active-versus-fixed-scan comparison, is open: it stopped at its paired-noise
integrity gate before the full trials. No registration, meshing or multi-object
