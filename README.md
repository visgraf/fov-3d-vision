...
| Step | State |
| --- | --- |
| FSG1: single-patch local stereo | Run 2026-09-19 (Blender 5.2.1, OPTIX), small then full under D-FSG1a, then a read-only audit under D-FSG1b. **FSG1 remains FAIL.** `small`/64 spp misses on the 3.4 m background's accuracy (1.14%/4.11%); `full`/256 spp meets every median and p95 in the suite but its 1.6 m foreground coverage is 79.3% against a 90% floor. The D-FSG1b audit replayed all three saved seed-17 records exactly, changed nothing, and added zero camera samples: 99.71% of that instance's rejections are a texture-only veto, and those discarded hypotheses score 0.178% median / 0.889% p95, as accurate as the accepted ones. Cause is over-exposure, not geometry - 92.5% of the cohort has all three RGB channels clipped, 51.8% is flat before quantization and 0.000% is flat in unclipped linear radiance, so the fixed display conversion destroys signal the render retained. Not adopted: raw SGBM on those pixels is degenerate (one propagated constant on a planar surface) and the veto catches genuinely bad hypotheses elsewhere (small/tilted, 34.5% over 3%). Half-occlusion is NOT EXERCISED in all nine case-records. Next step is a decision under D-FSG1/D-FSG1a/D-FSG1b. See `docs/fsg1-single-patch.md` and `docs/fsg1-coverage-audit.md`; prompts: `docs/fsg1-code-prompt.md`, `docs/fsg1-full-profile-followup.md`. |
