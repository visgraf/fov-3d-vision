...
| FSG4: active vs fixed scan (Increment 4) | **OPEN - stopped at the paired smoke on an integrity failure 2026-09-19; Increment 4 is NOT closed.** `fsg4_pair.py` exit 1: `paired observation differs at shared yaw -10.0`. The four full paired trials and the AUC comparison were NOT run, so no `FSG4_INCREMENT4_PASS/FAIL` exists. Diagnosed: at every shared yaw the Cycles seeds are identical and the oracle masks bit-exact; only RGB differs by one to two float32 ulp (max 5.96e-07). Rendering the **identical** command twice reproduces the same discrepancy as two renders at different steps, with identical seeds in all three - so the paired-seed rule is step-independent as written and the cause is non-associative GPU floating-point accumulation, not policy leakage. At the shared yaw every reconstruction statistic is bit-identical between policies. Not fixed locally: `fsg4_pair.py` faithfully implements the written "exactly identical" requirement, so this is a specification question, and relaxing it would change gate C5 and blunt the `--negative pairing` control. Nothing was altered. The decision for Luiz/Chat is whether the pairing criterion stays bit-exact (needing a deterministic rendering path) or is restated as bit-exact seeds and masks plus an explicit RGB tolerance. See `docs/fsg4-increment4.md`. |
