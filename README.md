...
| Step | State |
| --- | --- |
| FSG1: single-patch local stereo | Run 2026-09-19 (Blender 5.2.1, OPTIX): small, then full under D-FSG1a, a read-only audit under D-FSG1b, and one fixed encoding candidate under D-FSG1c. **The FSG1 baseline still FAILS and nothing is adopted.** `small`/64 spp misses on the 3.4 m background's accuracy (1.14%/4.11%); `full`/256 spp meets every accuracy target but its 1.6 m foreground coverage is 79.3% against a 90% floor. FSG1b traced that to over-exposure, not geometry: 99.71% of the rejections are a texture-only veto, 92.5% of that cohort has all three RGB channels clipped, and 0.000% is flat in unclipped linear radiance. FSG1c tested one fixed encoding, `sRGB(max(x,0)/(1+max(x,0)))`, on the SAME saved records with zero new samples: the full record becomes `CANDIDATE_PASS_ON_EXISTING_RECORD` (foreground coverage 79.3% -> 99.4%, all errors still inside 1%/3%, 8 pixels lost), while the small background miss is untouched. Not adopted and not validated: it is development-set evidence on the records that selected it, the analytic bright-full stress still fails coverage under the same encoding, small/tilted shows it accepting 29 weak pixels at 1.91%/4.08%, and half-occlusion is NOT_EXERCISED in all nine case-records. Next would be a held-out geometry/seed and a mirrored-step or opposite-eye fixture before any fusion. See `docs/fsg1-single-patch.md`, `docs/fsg1-coverage-audit.md`, `docs/fsg1-hdr-candidate.md`. |
