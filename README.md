...
| Step | State |
| --- | --- |
| FSG1: single-patch local stereo | Run 2026-09-19 (Blender 5.2.1, OPTIX) across D-FSG1a/b/c/d/e. **FSG1 is not closed, nothing is adopted, no fusion.** Seed-17 baselines FAIL; FSG1b traced the full-profile coverage miss to over-exposure, and FSG1c's fixed encoding `sRGB(max(x,0)/(1+max(x,0)))` cleared it on those records (development-set only). FSG1d then tested that frozen candidate prospectively on two NEW fixtures at seeds 31/73: `FROZEN_CANDIDATE_VALIDATION_FAIL` - the new tilted plane and 1.8 m foreground pass, the 3.2 m background misses accuracy on both seeds, and the half-occlusion check (EXERCISED for the first time, 4,608 raw / 3,528 core) leaks 2 core pixels on seed 31. FSG1e, a read-only replay of all 7 full pairs with zero new samples, then explained both: the background's RAW SGBM is 0.099%/0.359% and the bounded refinement degrades it monotonically to the +/-0.75 px caps at 23.25 and 24.75 px (~11% of accepted, ~88% of all >3% errors, matching the predicted 3.1234%/3.1265% to 14 digits) because the gradient denominator is ill-conditioned - but refinement RESCUES the original 3.4 m background (1.620% -> 0.785%), so it cannot simply be removed. All six accepted half-occluded points fail an endpoint check that the interpolated LR residual hides, though one is a genuinely self-consistent wrong cycle, so endpoint checking alone is insufficient. Next step is a decision by Luiz/Chat. See `docs/fsg1-single-patch.md`, `docs/fsg1-coverage-audit.md`, `docs/fsg1-hdr-candidate.md`, `docs/fsg1-prospective-validation.md`, `docs/fsg1-stage-visibility-audit.md`. |
