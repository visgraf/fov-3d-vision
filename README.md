...
| D2b | The neighbour test: a cell's distance from the median parallax of its consistent neighbours, as a sixth feature and as a rejection (`--nb-tol`), offline what-ifs, then the loop once if the offline rule is met | **written** 2026-09-18 — `stereo_field.py`, `active_loop.py`, `gross_diagnosis.py`, `docs/d2b-neighbour-test.md`; not run on the renders yet |
