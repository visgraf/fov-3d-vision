...
| Step | State |
| --- | --- |
| FSG1: single-patch local stereo | Run 2026-09-19 (Blender 5.2.1, OPTIX) across D-FSG1a/b/c/d. **FSG1 is not closed, nothing is adopted, no fusion.** Seed-17 baselines FAIL: `small` misses the 3.4 m background's accuracy (1.14%/4.11%), `full` meets every accuracy target but its 1.6 m foreground coverage is 79.3% vs a 90% floor. FSG1b traced that to over-exposure, not geometry (99.71% texture-only rejections; 92.5% all-channel clipped; 0.000% flat in unclipped linear). FSG1c's fixed encoding `sRGB(max(x,0)/(1+max(x,0)))` cleared it on those same records (79.3% -> 99.4%), development-set evidence only. FSG1d then tested that frozen candidate prospectively on two NEW fixtures at seeds 31/73 (865,075,200 samples): `FROZEN_CANDIDATE_VALIDATION_FAIL`. The new tilted plane passes on both seeds (99.76%/0.25%/1.11%) and the candidate's coverage gain reproduces on the new 1.8 m foreground (96.96% -> 99.87%), but the 3.2 m background misses accuracy on both seeds for BOTH estimators (1.053%/3.123%), and the half-occlusion check - EXERCISED for the first time, 4,608 raw / 3,528 core - leaks on seed 31, where both estimators accept 2 core pixels whose truth is 3.20 m while every existing veto passes. Next step is a decision by Luiz/Chat. See `docs/fsg1-single-patch.md`, `docs/fsg1-coverage-audit.md`, `docs/fsg1-hdr-candidate.md`, `docs/fsg1-prospective-validation.md`. |
