...
| Step | State |
| --- | --- |
| FSG1: single-patch local stereo | Run 2026-09-19 (Blender 5.2.1, OPTIX), small then full under D-FSG1a. 24 software checks pass, both deliberate negatives fail as designed, and independent Blender geometry checks pass by 3-4 orders of magnitude at both profiles. Both suites FAIL the 90%/1%/3% interior gate, on different criteria. `small`/64 spp: fronto and tilted pass; `step` misses on the 3.4 m background's accuracy (1.14%/4.11%). `full`/256 spp: every median and p95 in the suite is now inside the gate and that background passes (0.785%/2.497%), but the 1.6 m foreground's coverage falls 90.4% to 79.3% - 99.7% of its rejections are the fixed `std<0.5` textureless cutoff, which does not scale as doubled resolution halves per-pixel contrast. No `MILESTONE_PASS`. Half-occlusion is NOT EXERCISED (zero singly-visible reference pixels), a test-coverage gap for Chat to close before fusion. Nothing tuned; next step is a decision under D-FSG1/D-FSG1a. See `docs/fsg1-single-patch.md`; prompts: `docs/fsg1-code-prompt.md`, `docs/fsg1-full-profile-followup.md`. |
