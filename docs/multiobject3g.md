...
> **Reproduction note (2026-09-22).** The MultiObject-3g package was re-applied as
> commit `529ac79`, which restored this document to its as-applied state and thereby
> removed the Results section written at `db0b200`. The five MultiObject-3g tool
> sources are **byte-identical** to the ones that produced the record
> (`multiobject3g_public.py` `e5f82229a959440f…`, `multiobject3g_progress.py`
> `95cd5cf02efe9d68…`, `multiobject3g_run.py` `0eb7382b836e0771…`,
> `multiobject3g_compare.py` `277bcdd788d7d933…`, `check_multiobject3g.py`
> `4facc4ca78166868…`), and the contract text above is byte-identical to the original
> apply (`git diff c955696 529ac79 -- docs/multiobject3g.md` is empty). The completed
> record `previews/multiobject3g/full-seed2111` was **not** re-run — doing so would
> create a duplicate record and break the one-record-per-schema manifest scan that
> later increments rely on. Its numbers were re-verified unchanged and the checks,
> ten negatives and comparator were re-run green. The Results below are restored
> verbatim from `db0b200`.
